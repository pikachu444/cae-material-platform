...
def test_user_guide_navigation_links_and_screenshot_evidence_are_current() -> None:
    ...
    assert report.capture_count == 119
    ...
    assert report.duplicate_image_group_count == 323
...
def test_current_manifest_has_one_current_provenance_record_per_capture() -> None:
    ...
    assert manifest["version"] == 111
    assert manifest["scope"] == "issue-259-fe04e-modeling-fit-candidate-ui-extraction"
    assert re.fullmatch(r"[0-9a-f]{40}\+issue259-fe04e-worktree", current_source)
    assert manifest["source_commit"] == current_source
    assert "--only-modeling-process-fit" in manifest["capture_command"]
...
def test_current_images_are_product_routes_and_storybook_captures_are_untracked() -> None:
    ...
    assert len(manifest["captures"]) == 119
    ...
    assert len(list(current_images.glob("*.png"))) == 119
