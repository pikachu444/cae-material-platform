"""Pin the exact Recipe and successful Batch attempt behind promoted polymer Output.

Revision ID: 20260915_080_t69_origin
Revises: 20260914_079_t67_polymer

Traceability: ADR-0033, T-69.
"""
...
revision: str = "20260915_080_t69_origin"
down_revision: str | None = "20260914_079_t67_polymer"
...
_RECIPE_DIGEST = "705e6ca117a42552727b50fb7c0999bb22e4a13c1f1a31ec4afd10d10c248732"


def _family_constraint(*, include_recipe: bool) -> str:
...
def upgrade() -> None:
    op.execute(
        ...
        + _family_constraint(include_recipe=True)
        + ")"
    )
    op.execute(
        """
        ALTER TABLE modeling.linear_viscoelastic_processing_evidence
          ...
          ADD CONSTRAINT ck_modeling_linear_prony_recipe_batch_all_or_none CHECK (
            ...
            (processing_recipe_id IS NOT NULL AND processing_recipe_revision_id IS NOT NULL AND
             ...
             processing_batch_attempt_id IS NOT NULL AND processing_batch_attempt_no >= 1)
          ),
          ADD CONSTRAINT fk_modeling_linear_prony_recipe_exact FOREIGN KEY
          ...
          ADD CONSTRAINT fk_modeling_linear_prony_batch_exact FOREIGN KEY
            ...
            ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED,
          ADD CONSTRAINT fk_modeling_linear_prony_batch_member_exact FOREIGN KEY
            ...
            ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED,
          ADD CONSTRAINT fk_modeling_linear_prony_batch_attempt_exact FOREIGN KEY
            ...
            ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED;
        CREATE INDEX ix_mdl_linear_prony_processing_recipe ON
        ...
        CREATE FUNCTION modeling.validate_linear_prony_recipe_batch_origin()
        ...
        BEGIN
          ...
          IF origin.recipe_id IS DISTINCT FROM NEW.processing_recipe_id OR
            ...
            RAISE EXCEPTION 'linear Prony IR differs from exact Recipe/Batch execution origin'
              USING ERRCODE='23514';
          END IF;
          RETURN NEW;
        END $$;
        CREATE CONSTRAINT TRIGGER modeling_linear_prony_recipe_batch_origin_validate
          AFTER INSERT ON modeling.linear_viscoelastic_processing_evidence
          DEFERRABLE INITIALLY DEFERRED FOR EACH ROW
          EXECUTE FUNCTION modeling.validate_linear_prony_recipe_batch_origin();
        """
    )
...
def downgrade() -> None:
        ...
        DO $$ BEGIN
          IF EXISTS (
            SELECT 1 FROM modeling.linear_viscoelastic_processing_evidence
             WHERE processing_recipe_id IS NOT NULL
          ) THEN
            RAISE EXCEPTION 'cannot downgrade while immutable Recipe/Batch evidence exists';
          END IF;
        END $$;
        DROP TRIGGER modeling_linear_prony_recipe_batch_origin_validate
          ON modeling.linear_viscoelastic_processing_evidence;
        DROP FUNCTION modeling.validate_linear_prony_recipe_batch_origin();
        DROP INDEX modeling.ix_mdl_linear_prony_processing_recipe;
        ALTER TABLE modeling.linear_viscoelastic_processing_evidence
          DROP CONSTRAINT fk_modeling_linear_prony_batch_attempt_exact,
          DROP CONSTRAINT fk_modeling_linear_prony_batch_member_exact,
          DROP CONSTRAINT fk_modeling_linear_prony_batch_exact,
          DROP CONSTRAINT fk_modeling_linear_prony_recipe_exact,
          DROP CONSTRAINT ck_modeling_linear_prony_recipe_batch_all_or_none,
    ...
    op.execute(
        ...
        "DROP CONSTRAINT ck_modeling_material_model_family_digest, "
        "ADD CONSTRAINT ck_modeling_material_model_family_digest CHECK ("
        + _family_constraint(include_recipe=False)
        + ")"
    )
