"""Persist solver cards from exact Ogden-Prony reference IR revisions."""
...
OGDEN_PRONY_CARD_SCHEMA_ID = "urn:cmp:exporting:reference-ogden-prony-card:1.0.0"
OGDEN_PRONY_CARD_SCHEMA_VERSION = "1.0.0"


@dataclass(frozen=True, slots=True)
class CreateReferenceOgdenPronySolverCard:
...
@dataclass(frozen=True, slots=True)
class OgdenPronySolverCardSnapshot:
    id: UUID
    material_model_id: UUID
    target: OgdenPronyExportTarget
    solver_material_id: int
    material_name: str
    current: RevisionSnapshot[ReferenceOgdenPronySolverCardContent]


class OgdenPronyExportingRepository(Protocol):
...
def _require_decision(
    ...
    if (
        ...
        raise OgdenPronySolverCardConflict(
            "authorization decision does not match Ogden-Prony export request"
        )
...
class OgdenPronySolverCardService:
    def __init__(
        self,
        *,
        repository: OgdenPronyExportingRepository,
        material_models: OgdenPronyModelService,
    ...
    def create_card(
        ...
        record = RevisionService(
            ...
            CreateRevisionedAggregate(
                ...
                schema_id=OGDEN_PRONY_CARD_SCHEMA_ID,
                schema_version=OGDEN_PRONY_CARD_SCHEMA_VERSION,
                content=content,
