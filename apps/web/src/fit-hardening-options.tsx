...
export function HardeningFitOptions({ step, onChange, graphInteraction, onGraphModeChange, onApplyGraphSelection }: Props) {
  ...
  return <div className="guided-step-options">
    <fieldset className="fit-control-group candidate-check-grid"><legend>Candidate models</legend>{FAMILIES.map((family) => <label key={family.value}><input aria-label={`Include ${family.label} candidate`} type="checkbox" checked={families.includes(family.value)} onChange={() => toggle(family.value)} />{family.label}</label>)}</fieldset>
    <fieldset className="fit-control-group"><legend>Fit range</legend><div className="guided-range-row"><label>Start<input type="number" step="any" value={numeric(step, "fit_minimum_strain")} onChange={(event) => onChange("fit_minimum_strain", Number(event.target.value))}/></label><label>End<input type="number" step="any" value={numeric(step, "fit_maximum_strain")} onChange={(event) => onChange("fit_maximum_strain", Number(event.target.value))}/></label></div></fieldset>
    <fieldset className="fit-control-group selected-blend-group"><legend>Preview blend</legend><div className="fit-paired-controls"><label>Primary<select aria-label="Primary hardening law" value={String(step.options.primary_family)} onChange={(event) => onChange("primary_family", event.target.value)}>{FAMILIES.map((family) => <option key={family.value} value={family.value}>{family.label}</option>)}</select></label><label>Secondary<select aria-label="Secondary hardening law" value={String(step.options.secondary_family)} onChange={(event) => onChange("secondary_family", event.target.value)}>{FAMILIES.map((family) => <option key={family.value} value={family.value}>{family.label}</option>)}</select></label></div></fieldset>
    <fieldset className="fit-control-group contribution-group"><legend>Blend ratio</legend><div className="contribution-control-stack"><label className="slider-option"><span><input aria-label="Primary hardening contribution" type="range" min="0" max="1" step="0.01" value={numeric(step, "primary_weight")} onChange={(event) => onChange("primary_weight", Number(event.target.value))}/><output>{Math.round(numeric(step, "primary_weight") * 100)}%</output></span></label></div></fieldset>
    <fieldset className="fit-control-group extrapolation-group"><legend>Output range</legend><div className="fit-paired-controls"><label>Target strain<input type="number" min="0" max="5" step="0.01" value={numeric(step, "extrapolation_maximum_strain")} onChange={(event) => onChange("extrapolation_maximum_strain", Number(event.target.value))}/></label><label>Output points<input aria-label="Output points" type="number" min="21" max="501" step="1" value={numeric(step, "output_point_count")} onChange={(event) => onChange("output_point_count", Number(event.target.value))}/></label></div></fieldset>
    <fieldset className="fit-control-group graph-interaction-group"><legend>Graph selection</legend><div><button type="button" disabled={!graphInteraction.available} className={graphInteraction.mode === "range" ? "active" : ""} aria-pressed={graphInteraction.mode === "range"} onClick={() => onGraphModeChange(graphInteraction.mode === "range" ? "pan" : "range")}>Select range</button><button type="button" disabled={!graphInteraction.available} className={graphInteraction.mode === "point" ? "active" : ""} aria-pressed={graphInteraction.mode === "point"} onClick={() => onGraphModeChange(graphInteraction.mode === "point" ? "pan" : "point")}>Pick point</button>{graphInteraction.canApply ? <button type="button" onClick={onApplyGraphSelection}>Apply</button> : null}</div></fieldset>
  </div>;
}
