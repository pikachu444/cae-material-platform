...
export function HardeningFitOptions({ step, onChange, graphInteraction, onGraphModeChange, onApplyGraphSelection }: Props) {
  ...
  return <div className="guided-step-options">
    ...
    <fieldset className="fit-control-group"><legend>Selected blend</legend><label>Primary<select aria-label="Primary hardening law" value={String(step.options.primary_family)} onChange={(event) => onChange("primary_family", event.target.value)}>{FAMILIES.map((family) => <option key={family.value} value={family.value}>{family.label}</option>)}</select></label></fieldset>
