...
export function HardeningFitOptions({ step, onChange, graphInteraction, onGraphModeChange, onApplyGraphSelection }: Props) {
  ...
  return <div className="guided-step-options">
    ...
    <fieldset className="fit-control-group selected-blend-group"><legend>Selected blend</legend><div className="fit-paired-controls"><label>Primary<select aria-label="Primary hardening law" value={String(step.options.primary_family)} onChange={(event) => onChange("primary_family", event.target.value)}>{FAMILIES.map((family) => <option key={family.value} value={family.value}>{family.label}</option>)}</select></label><label>Secondary<select aria-label="Secondary hardening law" value={String(step.options.secondary_family)} onChange={(event) => onChange("secondary_family", event.target.value)}>{FAMILIES.map((family) => <option key={family.value} value={family.value}>{family.label}</option>)}</select></label></div></fieldset>
    <fieldset className="fit-control-group contribution-group"><legend>Primary contribution</legend><div className="contribution-control-stack"><label className="slider-option"><span><input aria-label="Primary hardening contribution" type="range" min="0" max="1" step="0.01" value={numeric(step, "primary_weight")} onChange={(event) => onChange("primary_weight", Number(event.target.value))}/><output>{Math.round(numeric(step, "primary_weight") * 100)}%</output></span></label><label>Review metric<select aria-label="Fit review metric" value="relative_rmse" disabled><option value="relative_rmse">Relative RMSE</option></select></label></div></fieldset>
    <fieldset className="fit-control-group extrapolation-group"><legend>Extrapolation</legend><div className="fit-paired-controls"><label>Target strain<input type="number" min="0" max="5" step="0.01" value={numeric(step, "extrapolation_maximum_strain")} onChange={(event) => onChange("extrapolation_maximum_strain", Number(event.target.value))}/></label><label>Output points<input aria-label="Output points" type="number" min="21" max="501" step="1" value={numeric(step, "output_point_count")} onChange={(event) => onChange("output_point_count", Number(event.target.value))}/></label></div></fieldset>
