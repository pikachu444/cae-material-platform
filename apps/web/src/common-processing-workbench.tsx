...
export function CommonProcessingWorkbench({ config, onNavigate, onModelingTrackChange, initialSession, onSessionChange, onSessionEvent, onNewSession, familyWorkbench, familyInspector, material, materialState, propertySet, locationSearch = "" }: Props) {
  ...
  return (
    <main className="processing-workbench-page">
      ...
      {notice && workflowTask !== "export" ? <div className="modeling-notice-line" role="status" title={notice}>{notice}</div> : null}
      ...
      {!elastomerWorkbenchTask && workflowTask !== "validate" && workflowTask !== "review" ? <section className={`workbench-card method-builder-card stage-${workflowTask}`} id="modeling-process">
        ...
        <ModelingWorkspaceLayout
          ...
          plot={<article className="persistent-modeling-plot" id="modeling-fit">
            <div className="section-heading"><div><p className="workspace-caption">{workflowTask === "data" ? "Source preview" : workflowTask === "process" ? "Processing preview" : workflowTask === "fit" ? "Candidate comparison" : "Selected model response"}</p><h2>{activePlotView === "ensemble" ? "Replicate statistics" : workflowTask === "export" ? "Test data & selected model" : activeStage ? methods.find((method) => method.method_id === activeStage.method_id)?.label ?? methodDisplayName(activeStage.method_id) : "Load data and preview"}</h2></div>{workflowTask !== "export" ? <div className="plot-view-switch" role="group" aria-label="Curve plot view"><button type="button" className={activePlotView === "pipeline" ? "active" : ""} disabled={!preview} onClick={() => setPlotView("pipeline")}>Pipeline</button><button type="button" className={activePlotView === "ensemble" ? "active" : ""} disabled={!ensemblePreview} onClick={() => setPlotView("ensemble")}>Mean &amp; band</button>{preview || ensemblePreview ? <span className="plot-preview-state">Preview — not saved</span> : null}</div> : preview ? <span className="plot-preview-state">Current saved fit</span> : null}</div>
