...
export function CommonProcessingWorkbench({ config, onNavigate, onModelingTrackChange, initialSession, onSessionChange, onSessionEvent, onNewSession, familyWorkbench, familyInspector, material, materialState, propertySet, locationSearch = "" }: Props) {
  ...
  const [visibleDocumentIds, setVisibleDocumentIds] = useState<string[]>([]);
  ...
  async function loadDocument(id: string): Promise<void> {
    ...
    try {
      ...
      setNotice(`Loaded saved dataset revision ${item.current_revision.revision_no}.`);
      setError(null);
    } catch (caught) {
  ...
  useEffect(() => {
    ...
    setVisibleDocumentIds((current) => {
      const compatible = current.filter((id) => exactIds.has(id));
      return compatible.length ? compatible : trackDocuments.map((item) => item.test_data_document_id);
    });
  ...
  const [stageTitle, stageRail, stageAction] = workflowTask === "data"
    ...
    : workflowTask === "process"
      ...
      : workflowTask === "fit"
      ? ["Fit material response", `Process · ${displayedRailEntries.length} steps`, "Run fit"]
  ...
  return (
    <main className="processing-workbench-page">
      <header className="modeling-context-strip" aria-label="Modeling context">
        <div className="modeling-work-title"><strong>{stageTitle}</strong><span>{[initialSession?.material?.label, selectedTrackDocument?.specimen_id || selectedTrackDocument?.document_key].filter(Boolean).join(" · ") || "Select test data"}</span></div>
      ...
      {!elastomerWorkbenchTask && workflowTask !== "validate" && workflowTask !== "review" ? <section className={`workbench-card method-builder-card stage-${workflowTask}`} id="modeling-process">
        ...
        <ModelingWorkspaceLayout
          navigator={workflowTask === "data" || workflowTask === "process" || workflowTask === "fit" ? <>
            <div className="modeling-dataset-list"><div className="rail-heading"><p>Curves</p><span>{trackDocuments.length} curves · {ensembleDocumentIds.length} included</span></div><label className="curve-filter"><span className="visually-hidden">Filter curves</span><input aria-label="Filter curves" placeholder="Filter curves" value={curveFilter} onChange={(event) => setCurveFilter(event.target.value)} /></label>{groupedTrackDocuments.map(([groupLabel, groupDocuments]) => <section className="curve-tree-group" aria-label={groupLabel} key={groupLabel}><details open><summary className="curve-group-row"><strong>{groupLabel}</strong><small>{groupDocuments.length}</small></summary>{groupDocuments.map((item) => { const label = curveDisplayName(item); const visible = visibleDocumentIds.includes(item.test_data_document_id); const curveIndex = trackDocuments.indexOf(item); return <article className={selectedDocumentId === item.test_data_document_id ? "active" : ""} key={item.test_data_document_id}><label className="curve-include-toggle" title="Include this revision in processing and fit"><input aria-label={`Include ${label} in processing and fit`} type="checkbox" checked={ensembleDocumentIds.includes(item.test_data_document_id)} onChange={() => toggleEnsembleDocument(item.test_data_document_id)}/></label><button type="button" className="curve-row-label" title={`${item.document_key} · ${item.specimen_id} · revision r${item.current_revision.revision_no}`} onClick={() => { setPlotView("pipeline"); void loadDocument(item.test_data_document_id); }}><span><strong>{label}</strong><small>r{item.current_revision.revision_no}</small></span></button><span className="dataset-curve-swatch" aria-label={`Plot color for ${label}`} role="img" style={{ "--curve-index": curveIndex } as React.CSSProperties}/><button type="button" className="curve-visibility-toggle" aria-pressed={visible} aria-label={`${visible ? "Hide" : "Show"} ${label} on plot`} title={`${visible ? "Hide" : "Show"} ${label} on plot`} onClick={() => setVisibleDocumentIds((current) => visible ? current.filter((id) => id !== item.test_data_document_id) : [...current, item.test_data_document_id])}><svg aria-hidden="true" viewBox="0 0 24 24" focusable="false"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.75"/>{!visible ? <path d="M3 3l18 18"/> : null}</svg></button></article>; })}</details></section>)}{!filteredTrackDocuments.length ? <p className="muted">No matching curves.</p> : null}{workflowTask === "process" && ensembleDocumentIds.length >= 2 ? <details className="rail-statistics-action"><summary>Replicate analysis</summary><label>Alignment points<input aria-label="Replicate alignment point count" type="number" min="5" max="1001" value={ensemblePointCount} onChange={(event) => { setEnsemblePointCount(Number(event.target.value)); setEnsemblePreview(null); }}/></label><button className="button secondary" type="button" disabled={busy} onClick={() => void runEnsemblePreview()}>{busy ? "Calculating…" : "Preview mean & band"}</button><small>Compatible observed-domain intersection only · no extrapolation</small></details> : null}</div>
            {workflowTask === "process" || workflowTask === "fit" ? <div className="configured-step-list"><p className="rail-title">{stageRail}</p>{displayedRailEntries.map(({ step, index, label, title, railIndex }) => { const groupedFitRail = workflowTask === "fit" && modelingTrack === "metal"; return <button type="button" title={title} className={selectedStepIndex === index ? "active" : ""} key={`${index}:${step.method_id}`} onClick={() => focusConfiguredStep(index)}><span>{groupedFitRail ? railIndex + 1 : index + 1}</span><span><strong>{label}</strong>{!groupedFitRail ? <small>{step.method_version}</small> : null}</span></button>; })}</div> : null}
          </> : null}
