...
export function MaterialSearchPage({ config, onNavigate, locationSearch }: Props) {
  ...
  const navigator = <aside className="materials-left-pane" aria-label={leftMode === "filters" ? "Material filters" : "Materials Browse Tree"}>
    <nav className="materials-navigator-tabs" aria-label="Materials navigator">
      <button type="button" aria-current={leftMode === "filters" ? "page" : undefined} onClick={() => setLeftMode("filters")}>Search</button>
      <button type="button" aria-current={leftMode === "browse" ? "page" : undefined} onClick={() => openBrowseTree(selectedExperience?.graph?.root)}>Browse</button>
      <button type="button" aria-current={leftMode === "subsets" ? "page" : undefined} onClick={() => setLeftMode("subsets")}>Subsets</button>
    </nav>
  ...
  const results = <section className="materials-results" aria-labelledby="material-results-title" aria-busy={loading}>
    ...
    <div className="materials-result-table-wrap"><table className="materials-result-table" aria-label="Material results"><colgroup>{Object.entries(columnWidths).map(([key, width]) => <col key={key} style={{ width }} />)}</colgroup><thead><tr><th>Compare<EngineeringColumnResizeHandle label="Compare" width={columnWidths.compare} min={60} max={100} onChange={(width) => setColumnWidths((current) => ({ ...current, compare: width }))}/></th><th><button type="button" aria-sort={sortKey === "name" ? sortDirection : undefined} onClick={() => changeSort("name")}>Material</button><EngineeringColumnResizeHandle label="Material" width={columnWidths.material} min={160} max={360} onChange={(width) => setColumnWidths((current) => ({ ...current, material: width }))}/></th><th><button type="button" aria-sort={sortKey === "family" ? sortDirection : undefined} onClick={() => changeSort("family")}>Family</button><EngineeringColumnResizeHandle label="Family" width={columnWidths.family} min={110} max={260} onChange={(width) => setColumnWidths((current) => ({ ...current, family: width }))}/></th><th>Source<EngineeringColumnResizeHandle label="Source" width={columnWidths.source} min={120} max={320} onChange={(width) => setColumnWidths((current) => ({ ...current, source: width }))}/></th><th><button type="button" aria-sort={sortKey === "yield" ? sortDirection : undefined} onClick={() => changeSort("yield")}>Yield</button><EngineeringColumnResizeHandle label="Yield" width={columnWidths.yield} min={90} max={180} onChange={(width) => setColumnWidths((current) => ({ ...current, yield: width }))}/></th><th><button type="button" aria-sort={sortKey === "cards" ? sortDirection : undefined} onClick={() => changeSort("cards")}>CAE cards</button><EngineeringColumnResizeHandle label="CAE cards" width={columnWidths.cards} min={90} max={180} onChange={(width) => setColumnWidths((current) => ({ ...current, cards: width }))}/></th></tr></thead><tbody>
...
function CardTable({ materialId, cards, downloadingId, onNavigate, onDownload }: { materialId: string; cards: SolverCardSummary[]; downloadingId: string; onNavigate: (path: string) => void; onDownload: (card: SolverCardSummary) => void }) {
  ...
  return <table className="ux-table cae-card-table"><thead><tr><th>Solver</th><th>Card</th><th>Format</th><th>Delivery</th></tr></thead><tbody>{cards.map((card) => <tr key={card.id}><td><strong>{card.solver}</strong></td><td title={card.label}>{card.label}</td><td>Native ASCII {card.extension}</td><td><div className="card-table-actions"><button className="ux-button tertiary" type="button" onClick={() => onNavigate(`/materials/${materialId}/cards/${card.id}`)}>Preview</button><button className="ux-button primary" type="button" disabled={downloadingId === card.id} onClick={() => onDownload(card)}>{downloadingId === card.id ? "Preparing…" : `Download ${card.extension}`}</button></div></td></tr>)}</tbody></table>;
}
