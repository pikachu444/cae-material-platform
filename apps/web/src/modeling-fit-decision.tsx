...
function SelectedParameterInspector({
  ...
  return <details className="selected-parameter-inspector" open>
    <summary>Selected parameter evidence ({parameters.length})</summary>
    {parameters.length ? <table className="fit-candidate-table" aria-label="Selected candidate parameters and bounds">
      <thead><tr><th scope="col">Law</th><th scope="col">Parameter</th><th scope="col">Value</th><th scope="col">Lower / upper bound</th></tr></thead>
      <tbody>{parameters.map((parameter) => <tr key={`${parameter.law}:${parameter.name}`}>
        <td>{parameter.law}</td>
        <th scope="row">{parameter.name}</th>
        <td>{displayEngineeringValue(parameter.value, parameter.unit)}</td>
        <td>{parameter.lower === null || parameter.upper === null
          ? "Server-computed · no editable bound"
          : `${displayEngineeringValue(parameter.lower, parameter.unit)} – ${displayEngineeringValue(parameter.upper, parameter.unit)}`}</td>
      </tr>)}</tbody>
...
function DecisionEditor({
  ...
  return <section className="selected-candidate-parameters" aria-label="Selected candidate decision">
    ...
    <p className="option-hint">Fit range: {selection.fitRange}. {selection.actualTermCount !== undefined ? `Server result selected ${selection.actualTermCount} term${selection.actualTermCount === 1 ? "" : "s"}; requested policy is ${selection.requestedTermPolicy ?? "not recorded"}.` : "Parameters and bounds remain in the contextual fit inspector."}</p>
...
export function HardeningCandidateEvidence({ stage, step, selection, onSelect, onChangeSelection }: {
  ...
  const evidence = families.map((family) => {
    ...
    const boundWarning = parameterKeys.some((key) => { const value = scalar.get(key); const lower = scalar.get(`${key}.lower`); const upper = scalar.get(`${key}.upper`); if (!value || !lower || !upper) return false; const span = upper.value - lower.value; return span > 0 && Math.min(value.value - lower.value, upper.value - value.value) / span < 0.001; });
    return { family, rmse, relative, boundWarning };
  ...
  const blend = primaryLaw && secondaryLaw && primaryLaw !== secondaryLaw
    ...
    ? {
        ...
        warning: evidence.some((candidate) => (candidate.family === primaryLaw || candidate.family === secondaryLaw) && candidate.boundWarning)
          ? "A contributing law has a parameter near its bound"
          : undefined,
      }
    : null;
  return <section className="hardening-candidate-evidence" aria-label="Hardening candidate numerical comparison"><div className="candidate-evidence-heading"><div><p className="eyebrow">Calculated candidates</p><h4>Fit evidence</h4></div><span>{evidence.length} equations{blend ? " + 1 blend" : ""}</span></div><div className="fit-candidate-table-wrap"><table className="fit-candidate-table" aria-label="Hardening candidate comparison"><thead><tr><th scope="col">Decision</th><th scope="col">Model / law</th><th scope="col">Recommendation</th><th scope="col">Metric</th><th scope="col">Fit / extrapolation range</th><th scope="col">Stability</th><th scope="col">Compatibility</th><th scope="col">Warning</th></tr></thead><tbody>{evidence.map((candidate) => { const selected = selection?.candidateKey === candidate.family; const warning = candidate.boundWarning ? "Parameter near bound" : undefined; return <tr className={selected ? "selected" : ""} key={candidate.family}><td><button type="button" aria-pressed={selected} aria-label={selected ? `${titleCase(candidate.family)} candidate selected` : `Select ${titleCase(candidate.family)} candidate`} onClick={() => onSelect({ candidateKey: candidate.family, displayLabel: titleCase(candidate.family), mode: "single", primaryLaw: candidate.family, reason: "", warningAcknowledged: false, fitRange, warning })}>{selected ? "Selected" : "Select candidate"}</button></td><th scope="row"><strong>{titleCase(candidate.family)}</strong></th><td>{candidate.family === recommended ? "Recommended · lowest relative RMSE" : "—"}</td><td><strong>{candidate.relative ? `${(candidate.relative.value * 100).toFixed(3)}%` : "—"}</strong><small>{candidate.rmse ? `RMSE · ${displayEngineeringValue(candidate.rmse.value, candidate.rmse.unit)}` : "No objective"}</small></td><td>{fitRange}</td><td>{candidate.boundWarning ? "Bound check" : "Converged"}</td><td>Reference hardening projection</td><td>{warning ?? "None"}</td></tr>; })}{blend ? <tr className={selection?.candidateKey === blend.candidateKey ? "selected" : ""}><td><button type="button" aria-pressed={selection?.candidateKey === blend.candidateKey} aria-label={selection?.candidateKey === blend.candidateKey ? `${blend.displayLabel} candidate selected` : `Select ${blend.displayLabel} candidate`} onClick={() => onSelect({ candidateKey: blend.candidateKey, displayLabel: blend.displayLabel, mode: "blend", primaryLaw: blend.primaryLaw, secondaryLaw: blend.secondaryLaw, primaryWeight: blend.primaryWeight, reason: "", warningAcknowledged: false, fitRange, warning: blend.warning })}>{selection?.candidateKey === blend.candidateKey ? "Selected" : "Select candidate"}</button></td><th scope="row"><strong>{blend.displayLabel}</strong><small>Calculated preview blend</small></th><td>—</td><td><strong>{blend.metric ? `${(blend.metric.value * 100).toFixed(3)}%` : "—"}</strong><small>Primary-law relative RMSE</small></td><td>{fitRange}</td><td>{blend.warning ? "Bound check" : "Converged"}</td><td>Exact preview blend identity</td><td>{blend.warning ?? "None"}</td></tr> : null}</tbody></table></div><DecisionEditor selection={selection} stage={stage} onChange={onChangeSelection}/></section>;
}
