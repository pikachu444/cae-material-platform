...
describe("Common Processing Workbench", () => {
  ...
  it("exposes the stale Recipe conflict actions after a rejected exact revision append", async () => {
    const sourceRef = { id: documentResource.test_data_document_id, revisionId: revision.id, label: documentResource.document_key, revisionNo: 1 };
    const recipeId = "recipe-conflict";
    const recipeRevisionId = "recipe-conflict-r1";
    const recipeHash = "c".repeat(64);
    const recipe = {
      ...
      content: {
        recipe_key: "metal-conflict-check",
        label: "Metal conflict check",
        description: "Exercise the exact stale revision recovery surface.",
        mapping_profile_id: mappingProfileResource.mapping_profile_id,
        mapping_profile_revision_id: mappingProfileResource.current_revision.id,
        mapping_profile_sha256: mappingProfileResource.current_revision.content_hash,
        steps: [
          { ordinal: 0, method_id: "rows.sort_unique", method_version: "1.0.0", options: {} },
          { ordinal: 1, method_id: "metal.elastic_modulus", method_version: "1.0.0", options: {} },
        ],
        lifecycle_state: "draft",
      },
    };
    const session = {
      ...
      materialFamily: "metal",
      objective: "Preserve a stale Recipe draft",
      material: { id: "material-a", revisionId: "material-a-r1", label: "DP600", revisionNo: 1 },
      materialState: { id: "state-a", revisionId: "state-a-r1", label: "As received", revisionNo: 1 },
      testData: sourceRef,
      mappingProfile: { id: mappingProfileResource.mapping_profile_id, revisionId: mappingProfileResource.current_revision.id, label: mappingProfileResource.content.label, revisionNo: 1 },
      recipe: { id: recipeId, revisionId: recipeRevisionId, label: recipe.content.label, revisionNo: 1 },
      workspace: {
        activeStage: "fit",
        selectedDocumentIds: [sourceRef.id],
        selectedTestDataRefs: [sourceRef],
    ...
    const fetchMock = vi.fn<typeof fetch>().mockImplementation(async (input, init) => {
      ...
      if (url.endsWith("/processing-methods")) return jsonResponse({ items: processMethodFixtures() });
      if (url.endsWith("/processing-outputs")) return jsonResponse({ items: [] });
      if (url.endsWith("/processing-ensemble-methods") || url.endsWith("/common-processing-batches")) return jsonResponse({ items: [] });
      if (url.endsWith("/common-processing-recipes")) return jsonResponse({ items: [recipe] });
      if (url.endsWith(`/common-processing-recipes/${recipeId}/revisions`) && init?.method === "POST") {
        revisionPosts += 1;
        expect(new Headers(init.headers).get("If-Match")).toBe(`"revision:1:sha256:${recipeHash}"`);
        return jsonResponse({ detail: "the Recipe head changed" }, 412);
      }
      if (url.includes("/test-data-documents/") && url.endsWith("/content")) return {
        ok: true,
        status: 200,
    ...
    const settingsControl = await screen.findByRole("button", { name: /current-stage settings/ }, { timeout: 5_000 });
    ...
    const savedRecipe = await screen.findByLabelText("Saved Processing Recipe");
    await waitFor(() => expect((savedRecipe as HTMLSelectElement).value).toBe(recipeId));
    fireEvent.click(screen.getByRole("button", { name: "Append draft revision" }));

    const conflict = await screen.findByRole("alert", { name: "Stale Recipe revision conflict" });
    expect(within(conflict).getByText("The Recipe changed after this draft was opened.")).toBeTruthy();
    expect(within(conflict).getByRole("button", { name: "Reload current" })).toBeTruthy();
    expect(within(conflict).getByRole("button", { name: "Keep local draft as new revision" })).toBeTruthy();
    expect(within(conflict).getByRole("button", { name: "Cancel" })).toBeTruthy();
