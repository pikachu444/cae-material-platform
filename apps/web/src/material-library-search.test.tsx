...
const material = (id: string, name: string) => ({
  ...
  current_revision: {
    ...
    organization_id: "org-1", project_id: "project-1", classification: "internal", lifecycle_state: "draft",
...
function catalogRecord(item: ReturnType<typeof material>) {
  ...
  return {
    ...
    domain_binding: { record_id: recordId, record_revision_id: recordRevisionId, kind: "material", object_id: item.material_id, revision_id: item.current_revision.id },
    current_revision: {
      ...
      content: {
        ...
        values: [
          { data_type: "discrete", attribute_definition_id: "material-class", value: item.current_revision.content.material_class },
          { data_type: "text", attribute_definition_id: "material-family", value: item.current_revision.content.material_family },
...
describe("MaterialSearchPage navigator and results", () => {
  ...
  it("defaults plain /materials to Browse, retains mode when finding, and keeps comparison local", async () => {
    ...
    const items = [material("material-1", "DP780"), material("material-2", "DP980")];
    vi.stubGlobal("fetch", vi.fn(async (input) => {
      const url = String(input);
      if (url.endsWith("/catalog/tables")) return response({ items: [{ table_id: "demo-material-records" }] });
      if (url.endsWith("/catalog/tables/demo-material-records/attributes")) return response({ items: catalogAttributes });
      if (url.endsWith("/catalog/records:search")) return response({
        items: items.map(catalogRecord), total_count: 2, offset: 0, limit: 50,
        facets: [
          { attribute_definition_id: "material-class", value: "metal", count: 2 },
          { attribute_definition_id: "provider", value: "Demo provider", count: 2 },
          { attribute_definition_id: "evidence-source", value: "Synthetic reference", count: 2 },
        ],
      });
      return response({});
    }));
    ...
    const observeWorkspaceStatus = (event: Event) => {
      workspaceStatusUpdates.push((event as CustomEvent<{ revision?: string }>).detail);
    };
    ...
    expect(await screen.findByRole("checkbox", { name: "Compare DP780" })).toBeTruthy();
    await waitFor(() => expect(workspaceStatusUpdates.some(({ revision }) => revision === "r3")).toBe(true));
    const selectedStatus = workspaceStatusUpdates.find(({ revision }) => revision === "r3");
    ...
    const materialSortButton = screen.getByRole("button", { name: "Material / grade" });
    ...
    const ascendingPath = materialSortButton.querySelector("svg path:last-child")?.getAttribute("d");
    fireEvent.click(materialSortButton);
    await waitFor(() => expect(window.location.search).toContain("direction=descending"));
    const descendingPath = materialSortButton.querySelector("svg path:last-child")?.getAttribute("d");
    ...
    fireEvent.click(screen.getByRole("button", { name: "Filters" }));
    const familySelect = await screen.findByRole("combobox", { name: "Material class" });
    ...
    await waitFor(() => expect(new URLSearchParams(window.location.search).get("family")).toBe("metal"));
  });
  ...
  it("restores Filters and Subsets from the URL without exposing shell commands", async () => {
    vi.stubGlobal("fetch", vi.fn(async (input) => {
      ...
      if (url.endsWith("/catalog/tables")) return response({ items: [{ table_id: "demo-material-records" }] });
      if (url.endsWith("/catalog/tables/demo-material-records/attributes")) return response({ items: catalogAttributes });
      if (url.endsWith("/catalog/records:search")) return response({ items: [], total_count: 0, offset: 0, limit: 50, facets: [] });
      return response({});
    }));
  ...
  it("opens the selected catalog row with distinct exact record and Material revision pins", async () => {
    vi.stubGlobal("fetch", vi.fn(async (input) => {
      ...
      if (url.endsWith("/catalog/tables/demo-material-records/attributes")) return response({ items: catalogAttributes });
      if (url.endsWith("/catalog/records:search")) return response({
        items: [catalogRecord(material("material-1", "DP780"))], total_count: 1, offset: 0, limit: 50, facets: [],
      });
      return response({});
    }));
    const onNavigate = vi.fn();
    render(<MaterialSearchPage config={{ baseUrl: "/api/v1", accessToken: "test-token" }} onNavigate={onNavigate} locationSearch="" />);

    const row = await screen.findByRole("row", { name: /DP780/ });
    fireEvent.keyDown(row, { key: "Enter" });
    expect(onNavigate).toHaveBeenLastCalledWith(expect.stringContaining("/materials/material-1?"));
    const enterUrl = new URL(onNavigate.mock.lastCall?.[0] ?? "/", "http://localhost");
