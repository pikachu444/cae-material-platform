...
이 폴더에는 테스트와 형식 검증을 같은 입력으로 다시 실행하기 위한 고정 자료가 있습니다. 생산
데이터의 보관소는 아닙니다. 승인된 공개 측정 자료는 라이선스와 출처가 명확하고 운영 입력으로
승격되지 않는 경우에만 `public/`에 원본 archive를 보존합니다.
...
| 공개 점탄성·인장 측정 자료의 반복 검증을 실행할 때 | [manifest 상세 안내](manifests/README.md) | 공개 원본 archive와 각 member의 구조·단위·사용 가능 범위를 함께 검증합니다. |
...
계산 기대값에는 합성한 비기밀·비운영 자료만 추가합니다. 공개 원본 측정 fixture는 정확한 활성
범위가 승인되고 재배포 라이선스가 확인된 자료만 허용하며, `public_reference_non_production` 분류와
원본 단위·출처를 유지합니다. 고객·공급자 자료와 기밀 자료는 fixture로 넣지 않습니다. 공개 측정값은
생산 표준, 재료 모델이나 검증 기준을 결정하지 않으며, 누락된 물성이나 단위를 보정해서도 안 됩니다.
고정된 archive를 바꿔야 한다면 기존 파일을 덮어쓰지 말고 승인된 새 버전을 추가합니다.
