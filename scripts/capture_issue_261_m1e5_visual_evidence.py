...
DEFAULT_MANIFEST = (
    ROOT / "docs/17-evidence/images/issue-261-m1e5-producer-routed-residual/manifest.json"
)
...
RENDERED_PRODUCER_SELECTORS = [
    (
        'section.neutral-solver-export[aria-label="Reviewed Neutral Material and solver card '
        'delivery"] .mapping-report-heading'
    ),
    (
        'section.neutral-solver-export[aria-label="Reviewed Neutral Material and solver card '
        'delivery"] .mapping-list'
    ),
]
SHA256_LENGTH = 64
...
CONDITIONAL_CURVE_GROUP_REASON = (
    "The normal producer SVG five-viewport route does not materialize the conditional "
    "active/tooltip state; CSS-0172\N{EN DASH}CSS-0176 remain source/bundle/component-tested "
    "with no DOM/state fabrication."
)
GOVERNED_IMPORT_0887_REASON = (
    "The normal governed-import producer does not materialize the conditional preview "
    ".curve-heading row; CSS-0887 remains source/component/bundle-tested and no live locator "
    "is claimed."
)
...
def validate_selector_application(manifest: dict[str, Any]) -> None:
    ...
    require(
        application.get("status_counts") == {"LIVE": 38, "N_A_SOURCE_TEST": 20, "RETAINED_HOLD": 2},
        "selector application counts drifted",
    )
    require(application.get("live_ids") == LIVE_SELECTOR_IDS, "LIVE selector partition drifted")
    na = application.get("na_source_test")
    require(
        isinstance(na, list) and [item.get("id") for item in na] == NA_SOURCE_TEST_SELECTOR_IDS,
        "N/A selector partition drifted",
    )
    conditional_curve_group = [item for item in na if item.get("id") in CONDITIONAL_CURVE_GROUP_IDS]
    require(
        [item.get("id") for item in conditional_curve_group] == CONDITIONAL_CURVE_GROUP_IDS,
        "conditional curve group partition drifted",
    )
    for item in conditional_curve_group:
        require(
            item.get("reason") == CONDITIONAL_CURVE_GROUP_REASON,
            f"{item.get('id')}: conditional group reason drifted",
        )
        require(
            "curve-contract-chart.tsx" in item.get("source_evidence", {}).get("source", ""),
            f"{item.get('id')}: source proof missing",
        )
        require(
            "curve-contract-chart.test.tsx" in item.get("source_evidence", {}).get("component", ""),
            f"{item.get('id')}: component-test proof missing",
        )
    governed_import_0887 = next((item for item in na if item.get("id") == "CSS-0887"), None)
    require(
        governed_import_0887 is not None
        and governed_import_0887.get("reason") == GOVERNED_IMPORT_0887_REASON,
        "CSS-0887 N/A reason drifted",
    )
    require(
        "design/primitives.css" in governed_import_0887["source_evidence"]["source"],
        "CSS-0887 source proof missing",
    )
    require(
        "governed-import-workbench.tsx" in governed_import_0887["source_evidence"]["component"],
        "CSS-0887 component proof missing",
    )
    require(
        "app.tsx" in governed_import_0887["source_evidence"]["import_chain"]
        and "governed-import-workbench" in governed_import_0887["source_evidence"]["import_chain"],
        "CSS-0887 import proof missing",
    )
    require(
        application.get("retained_hold_ids") == ["CSS-1446", "CSS-1447"],
        "retained hold selector partition drifted",
    )
    ...
    require(
        isinstance(properties, dict) and list(properties) == LIVE_SELECTOR_IDS,
        "intended property map drifted",
    )
    contracts = application.get("live_contracts")
    require(
        isinstance(contracts, list) and [item.get("id") for item in contracts] == LIVE_SELECTOR_IDS,
        "LIVE locator contract drifted",
    )
    observed_route_counts: dict[str, int] = {}
    for contract in contracts:
        require(
            isinstance(contract.get("locator"), str) and contract["locator"],
            f"{contract.get('id')}: LIVE locator required",
        )
        require(
            isinstance(contract.get("base_selector"), str) and contract["base_selector"],
            f"{contract.get('id')}: base selector required",
        )
        require(
            properties.get(contract["id"]) == target_rows[contract["id"]][8],
            f"{contract['id']}: intended properties must match frozen fixture",
        )
        if contract.get("route_id") == "modeling-data-metal":
            require(
                contract["locator"].startswith(".modeling-workspace-stage-data "),
                f"{contract['id']}: Data locator must use the visible stage scope",
            )
            require(
                ".modeling-data-plot" not in contract["locator"],
                f"{contract['id']}: stale Data plot scope",
            )
    ...
    for record in na:
        require(
            record.get("disposition") == "N/A_SOURCE_TEST",
            f"{record.get('id')}: N/A disposition drifted",
        )
        ...
        require(
            ...
            and all(
                ...
                for key in ("source", "component", "import_chain", "bundle")
            ),
            f"{record.get('id')}: exact source/component/import/bundle proof required",
        )
    ...
    require(
        focus_record["reason"]
        == (
            "The normal Materials curve SVG is captured at all five required viewports; the "
            "focus pseudo-state is source/bundle/component-tested per the owner "
            "verification-reduction direction and is not fabricated."
        ),
        "CSS-0158 N/A reason drifted",
    )
    require(
        "curve-contract-chart.test.tsx" in focus_record["source_evidence"]["component"],
        "CSS-0158 component-test proof is required",
    )
...
def validate_registered_images(manifest: dict[str, Any]) -> None:
    ...
    for ordinal, record in enumerate(registered, start=1):
        ...
        require(
            image.startswith("docs/17-evidence/images/issue-261-m1e5-producer-routed-residual/"),
            f"registered_images[{ordinal}] must stay in the M1E5 evidence root",
        )
        require(
            image.lower().endswith(".png"), f"registered_images[{ordinal}] must reference a PNG"
        )
        paths.append(image)
    ...
    require(
        paths == expected,
        "registered_images must exactly match PNG files under before/after evidence trees",
    )
...
def validate_duplicate_image_groups(manifest: dict[str, Any]) -> None:
    ...
    require(
        ...
        "registered_images must be a list before duplicate-group validation",
    )
    registered_paths = {
        record["image"]
    ...
    require(
        len(declared) == DUPLICATE_GROUP_COUNT,
        f"allowed_duplicate_groups must contain exactly {DUPLICATE_GROUP_COUNT} groups",
    )
    ...
    for ordinal, record in enumerate(declared, start=1):
        ...
        require(
            record.get("rationale") == DUPLICATE_GROUP_RATIONALE,
            f"allowed_duplicate_groups[{ordinal}] rationale drifted",
        )
        images = record.get("images")
        require(
            isinstance(images, list) and len(images) >= 2,
            f"allowed_duplicate_groups[{ordinal}].images must contain at least two paths",
        )
        require(
            all(isinstance(path, str) and path for path in images),
            f"allowed_duplicate_groups[{ordinal}].images must contain non-empty strings",
        )
        require(
            images == sorted(images),
            f"allowed_duplicate_groups[{ordinal}].images must be lexicographically sorted",
        )
        require(
            len(images) == len(set(images)),
            f"allowed_duplicate_groups[{ordinal}].images must contain unique paths",
        )
        first_path = images[0]
        require(
            previous_first is None or previous_first < first_path,
            "allowed_duplicate_groups must be deterministically ordered by first path",
        )
        previous_first = first_path
        require(
            not seen_paths.intersection(images),
            f"allowed_duplicate_groups[{ordinal}] overlaps a previous group",
        )
        seen_paths.update(images)
        require(
            any(path in registered_paths for path in images),
            f"allowed_duplicate_groups[{ordinal}] must include an M1E5 registered image",
        )
        ...
        for path_text in images:
            path = Path(path_text)
            require(
                not path.is_absolute() and path.suffix.lower() in IMAGE_EXTENSIONS,
                f"allowed_duplicate_groups[{ordinal}] contains an invalid image path",
            )
            candidate = ROOT / path
            try:
                ...
                raise ContractError(
                    f"allowed_duplicate_groups[{ordinal}] escapes the repository root"
                ) from exc
            require(
                relative == path_text and candidate.is_file(),
                f"allowed_duplicate_groups[{ordinal}] references a missing image: {path_text}",
            )
            digests.add(hashlib.sha256(candidate.read_bytes()).hexdigest())
        require(
            len(digests) == 1,
            f"allowed_duplicate_groups[{ordinal}] paths must share one SHA-256 digest",
        )
        declared_paths.append(images)

    expected = current_duplicate_image_groups(registered_paths)
    require(
        len(expected) == DUPLICATE_GROUP_COUNT,
        "current repository duplicate-image group count drifted",
    )
    expected_paths = [paths for _, paths in expected]
    require(
        declared_paths == expected_paths,
        (
            "allowed_duplicate_groups must exactly match current repository-wide duplicate "
            "groups containing M1E5 registered images"
        ),
    )
    require(
        sum(len(paths) for paths in declared_paths) == DUPLICATE_IMAGE_PATH_COUNT,
        "allowed_duplicate_groups image-path count drifted",
    )
    require(
        max(len(paths) for paths in declared_paths) == DUPLICATE_GROUP_MAX_SIZE,
        "allowed_duplicate_groups maximum group size drifted",
    )
    ...
    require(
        cross_evidence_count == DUPLICATE_GROUP_CROSS_EVIDENCE_COUNT,
        "allowed_duplicate_groups cross-evidence count drifted",
    )
...
def validate_acceptance(manifest: dict[str, Any]) -> None:
    ...
    require(
        manifest.get("status") == ACCEPTED_MANIFEST_STATUS, "accepted manifest status is required"
    )
    serialized = json.dumps(manifest, sort_keys=True)
    require(
        "PENDING_MAIN_CAPTURE" not in serialized,
        "accepted manifest contains pending capture evidence",
    )
    ...
    require(
        acceptance.get("owner_direction") == "OWNER_DIRECTION_APPLIED_MAIN_INSPECTED",
        "owner direction record drifted",
    )
    ...
    require(
        inventory.get("viewports_each") == 5, "each captured topology must cover five viewports"
    )
    require(
        ...
        == [
            {
                ...
                "equivalent_to": "modeling-process-metal",
                "viewport": "1440x900",
                "disposition": EQUIVALENCE_CAPTURE_EVIDENCE,
            },
            {
                "id": "modeling-alias-data",
                "equivalent_to": "modeling-data-metal",
                "viewport": "1440x900",
                "disposition": EQUIVALENCE_CAPTURE_EVIDENCE,
            },
        ],
    ...
    require(
        metrics.get("selector_computed_equality") == {"matched": 190, "total": 190},
        "selector computed equality drifted",
    )
    require(
        metrics.get("page_geometry") == {"matched": 35, "total": 35}, "page geometry result drifted"
    )
    require(
        metrics.get("crop_geometry_display_overflow") == {"matched": 160, "total": 160},
        "crop geometry result drifted",
    )
    require(
            ...
            "differences": [
                {
                    ...
                    "reason": (
                        "Seed-generated revision prefix 2a942de3 vs 2f24caad glyph width; "
                        "no CSS regression."
                    ),
                }
            ],
    ...
    require(
        metrics.get("png_byte_identity") == {"identical": 160, "total": 195},
        "PNG identity result drifted",
    )
    require(
    ...
    require(
        visual_review.get("representative_crops_opened") is True,
        "representative crops must be opened",
    )
    require(
        visual_review.get("crop_viewports") == ["1366x768", "3840x2160"],
        "representative crop viewports drifted",
    )
    require(
        visual_review.get("browser_zoom_percent") == 100
        and visual_review.get("device_pixel_ratio") == 1,
        "visual review zoom/DPR drifted",
    )
    require(
    ...
    require(
        visual_review.get("no_clipping_overflow_topology_regression") is True,
        "visual regression disposition drifted",
    )
    require(
        visual_review.get("physical_readability") == "DEFERRED_TO_223",
        "physical readability must remain deferred to #223",
    )
    ...
    require(
        ...
        == [
            {
                "id": "main-capture-tree-before",
                "phase": "before",
                "file_count": 230,
                "png_count": 195,
                "computed_json_count": 35,
                "tree_sha256": "aa04391a18abefa75333233f242ec10a5ef52710d057fdf2edf0eaac6991b795",
                "disposition": "PASS_MAIN_CAPTURE",
            },
            {
                "id": "main-capture-tree-after",
                "phase": "after",
                "file_count": 230,
                "png_count": 195,
                "computed_json_count": 35,
            ...
            {
                "id": "main-visual-comparison",
                "selector_computed_equality": "190/190",
                "page_geometry": "35/35",
                "crop_geometry_display_overflow": "160/160",
                "selector_geometry": "175/190",
                "png_byte_identity": "160/195",
                "disposition": "PASS_MAIN_VISUAL",
            },
            {
                "id": "main-runtime-topology",
                "captured_topologies": 7,
                "viewports_each": 5,
                "aliases": "2/2",
                "disposition": "PASS_MAIN_RUNTIME",
            },
        ],
    ...
    require(
        review.get("main_visual_runtime_acceptance") == "PASS_MAIN",
        "Main visual/runtime acceptance must be PASS",
    )
    require(
        review.get("product_owner_original_resolution_review")
        == "OWNER_DIRECTION_APPLIED_MAIN_INSPECTED",
        "owner direction review record drifted",
    )
    require(
        review.get("physical_windows_4k_readability") == "DEFERRED_TO_223",
        "physical readability status drifted",
    )
    require(
        isinstance(review.get("notes"), str)
        and "Physical Windows 4K readability remains deferred to #223" in review["notes"],
        "review notes must retain the #223 physical gate",
    )
...
def validate_manifest(manifest: dict[str, Any], path: Path) -> None:
    ...
    require(
        manifest.get("status") == ACCEPTED_MANIFEST_STATUS,
        "manifest must record accepted Main visual/runtime evidence",
    )
    ...
    require(
        manifest.get("viewports") == VIEWPORTS,
        "five required CSS viewports are incomplete or reordered",
    )
    ...
    for key in ("approved_tuple_sha256", "candidate_tuple_sha256", "retained_tuple_sha256"):
        ...
        require(
            isinstance(value, str) and len(value) == SHA256_LENGTH,
            f"ownership.{key} must be a SHA-256 digest",
        )
    ...
    require(
        capture_plan.get("required_viewports") == VIEWPORTS, "capture viewport matrix is incomplete"
    )
    require(
        set(capture_plan.get("required_crops", [])) == CROPS, "required direct crop set changed"
    )
    require(
        capture_plan.get("original_resolution_required") is True,
        "original-resolution review is required",
    )
    ...
    require(
        capture_plan.get("physical_readability") == "DEFERRED_TO_223",
        "physical 4K gate must remain deferred to #223",
    )
    ...
    require(
        topology.get("unique_topologies")
        == [
            "modeling-data-metal",
            "modeling-process-elastomer-hold",
            "materials-curves",
    ...
    require(
        topology.get("captured_target_topologies") == CAPTURED_TARGET_TOPOLOGY_IDS,
        "captured target topology accounting drifted",
    )
    no_screenshot = topology.get("no_screenshot_states")
    require(
        isinstance(no_screenshot, list)
        and [item.get("id") for item in no_screenshot]
        == [
            "modeling-process-metal",
            "modeling-fit-metal",
            "modeling-export-metal",
        ...
        "no-screenshot state accounting drifted",
    )
    for item in no_screenshot:
        require(
            item.get("disposition") == "N/A_REDUNDANT_NO_LIVE_TARGETS"
            and isinstance(item.get("reason"), str)
            and item["reason"],
            f"{item.get('id')}: no-screenshot reason is required",
        )
    require(
        topology.get("equivalence_groups")
        == [
            {
                "id": "modeling-data-metal",
                "canonical": "modeling-data-metal",
                "aliases": ["modeling-alias-data"],
                "source_routes": ["/modeling", "/datasets/processing"],
            },
            {
                "id": "modeling-process-metal",
                "canonical": "modeling-process-metal",
                "aliases": ["modeling-alias-process"],
                "source_routes": ["/modeling", "/datasets/processing"],
            },
        ],
    ...
    require(
        primary_journey.get("id") == "authenticated-modeling-data-process-handoff",
        "primary journey id drifted",
    )
    require(
        primary_journey.get("authentication")
        and "demo-identity/token" in primary_journey["authentication"],
        "primary journey must record demo authentication",
    )
    require(
        primary_journey.get("exact_fixture") == "CMP-DEMO-DP780-TEST-JSON@r1",
        "primary journey must pin the exact Test Data fixture",
    )
    sequence = primary_journey.get("route_sequence")
    require(
        isinstance(sequence, list)
        and [item.get("id") for item in sequence if isinstance(item, dict)] == PRIMARY_JOURNEY_IDS,
        "primary journey route sequence drifted",
    )
    require(
        primary_journey.get("coverage_role")
        == (
            "normal Data producer plus one representative no-screenshot Process handoff "
            "recovery; no Fit/Export mutation or screenshot claim"
        ),
        "primary journey must identify the common-workbench boundary",
    )
    require(
        primary_journey.get("evidence") == ACCEPTED_CAPTURE_EVIDENCE,
        "primary journey must record accepted capture evidence",
    )

    producer_coverage = capture_plan.get("producer_coverage")
    require(
        isinstance(producer_coverage, dict),
        "producer_coverage must partition rendered and unreachable producers",
    )
    rendered = producer_coverage.get("rendered")
    require(
        isinstance(rendered, list) and len(rendered) == 1,
        "producer_coverage.rendered must contain one supported producer journey",
    )
    rendered_journey = rendered[0]
    require(isinstance(rendered_journey, dict), "rendered producer journey must be an object")
    require(
        rendered_journey.get("id") == RENDERED_PRODUCER_JOURNEY_ID,
        "rendered producer journey id drifted",
    )
    require(
        rendered_journey.get("producer_paths") == [RENDERED_PRODUCER_PATH],
        "rendered producer path must be the Neutral export consumer",
    )
    require(
        rendered_journey.get("rendered_selectors") == RENDERED_PRODUCER_SELECTORS,
        "rendered producer selectors drifted",
    )
    ...
    require(
        isinstance(rendered_journey.get("authentication"), str)
        and "demo-identity/token" in rendered_journey["authentication"],
        "rendered producer journey must record demo authentication",
    )
    require(
        isinstance(rendered_journey.get("exact_fixture"), str)
        and "Synthetic Elastomer" in rendered_journey["exact_fixture"],
        "rendered producer journey must pin the synthetic elastomer fixture",
    )
    require(
        rendered_journey.get("evidence") == ACCEPTED_CAPTURE_EVIDENCE,
        "rendered producer journey must record accepted capture evidence",
    )
    unreachable = producer_coverage.get("unreachable")
    require(
        isinstance(unreachable, list) and len(unreachable) == 2,
        "producer_coverage.unreachable must record the two unsupported producer routes",
    )
    ...
    for item in unreachable:
        ...
        require(
            path_value in PRODUCER_PATHS,
            f"unreachable producer path is not in the M1E5 roster: {path_value}",
        )
        unreachable_paths.add(path_value)
        require(
            path_value != RENDERED_PRODUCER_PATH,
            "the rendered Neutral producer cannot also be unreachable",
        )
        require(
            item.get("status") == "UNREACHABLE_WITHOUT_PRODUCT_ROUTE",
            f"{path_value}: unsupported route status must be explicit",
        )
        evidence = item.get("source_evidence")
        require(
            isinstance(evidence, list) and evidence,
            f"{path_value}: source evidence is required for an unreachable producer",
        )
        require(
            isinstance(item.get("decision"), str)
            and "Do not claim rendered producer coverage" in item["decision"],
            f"{path_value}: unreachable decision must forbid false coverage",
        )
    require(
        unreachable_paths | {RENDERED_PRODUCER_PATH} == PRODUCER_PATHS,
        "producer coverage must partition all three mapping-report consumers exactly once",
    )

    route_matrix = capture_plan.get("route_matrix")
    require(
        isinstance(route_matrix, list), "route_matrix must enumerate every browser producer case"
    )
    route_ids = {route.get("id") for route in route_matrix if isinstance(route, dict)}
    require(
        route_ids == EXPECTED_ROUTE_IDS,
        "route_matrix ids drifted from the M1E5 browser producer matrix",
    )
    for route in route_matrix:
        require(isinstance(route, dict), "route_matrix records must be objects")
        require(
            isinstance(route.get("route"), str) and route["route"],
            f"{route.get('id')}: route is required",
        )
        require(
            isinstance(route.get("selectors"), list) and route["selectors"],
            f"{route.get('id')}: producer selectors are required",
        )
        require(
            route.get("classification") in CLASSIFICATIONS,
            f"{route.get('id')}: classification must be primary, technical, or negative",
        )
        if "rendered_selectors" in route:
            require(
                isinstance(route.get("rendered_selectors"), list) and route["rendered_selectors"],
                f"{route.get('id')}: rendered producer selectors must be non-empty",
            )
        if route.get("capture_disposition") in {"collapsed-equivalence", "no-screenshot-technical"}:
            require(
                route.get("required_viewports") == [] and route.get("required_crops") == [],
                f"{route.get('id')}: collapsed alias must not duplicate captures",
            )
            if route.get("capture_disposition") == "collapsed-equivalence":
                require(
                    route.get("equivalence_group")
                    and route.get("equivalent_to") == route.get("equivalence_group"),
                    f"{route.get('id')}: equivalence target is required",
                )
            else:
                require(
                    route.get("id") in NO_SCREENSHOT_STATE_IDS
                    and isinstance(route.get("reason"), str)
                    and route["reason"].startswith("N/A_REDUNDANT_NO_LIVE_TARGETS:"),
                    f"{route.get('id')}: no-screenshot reason is required",
                )
        else:
            require(
                route.get("required_viewports") == VIEWPORTS,
                f"{route.get('id')}: viewport matrix drifted",
            )
        crops = route.get("required_crops")
        require(
            isinstance(crops, list) and set(crops).issubset(CROPS),
            f"{route.get('id')}: crop set is invalid",
        )
        ...
        require(
            route.get("evidence") == expected_evidence,
            f"{route.get('id')}: evidence disposition drifted",
        )
    elastomer_route = next(
        (route for route in route_matrix if route.get("id") == "modeling-fit-elastomer"), None
    )
    require(isinstance(elastomer_route, dict), "modeling-fit-elastomer producer route is required")
    require(
        elastomer_route.get("classification") == "primary",
        "modeling-fit-elastomer must be the rendered primary producer route",
    )
    require(
        elastomer_route.get("rendered_selectors") == RENDERED_PRODUCER_SELECTORS,
        "modeling-fit-elastomer rendered selectors drifted",
    )
    require(
        elastomer_route.get("selectors") == [".mapping-report-heading", ".mapping-list"],
        "modeling-fit-elastomer CSSOM selectors must name the mapping report",
    )
    ...
    require(
        state_ids == EXPECTED_ROUTE_IDS,
        "state_matrix must enumerate the same route ids as route_matrix",
    )
    for state in states:
        require(isinstance(state, dict), "state records must be objects")
        require(
            state.get("classification") in CLASSIFICATIONS,
            f"{state.get('id')}: classification must be primary, technical, or negative",
        )
        ...
        require(
            state.get("evidence") == expected_evidence,
            f"{state.get('id')}: evidence disposition drifted",
        )
        if state.get("capture_disposition") in {"collapsed-equivalence", "no-screenshot-technical"}:
            require(
                state.get("required_viewports") == [] and state.get("required_crops") == [],
                f"{state.get('id')}: collapsed alias must not duplicate captures",
            )
            if state.get("capture_disposition") == "no-screenshot-technical":
                require(
                    state.get("id") in NO_SCREENSHOT_STATE_IDS
                    and isinstance(state.get("reason"), str)
                    and state["reason"].startswith("N/A_REDUNDANT_NO_LIVE_TARGETS:"),
                    f"{state.get('id')}: no-screenshot reason is required",
                )
        else:
            require(
                state.get("required_viewports") == VIEWPORTS,
                f"{state.get('id')}: viewport matrix drifted",
            )

    recovery_matrix = capture_plan.get("recovery_matrix")
    require(
        isinstance(recovery_matrix, list) and recovery_matrix,
        "recovery_matrix must record source-test/N/A recovery states",
    )
    for recovery in recovery_matrix:
        require(isinstance(recovery, dict), "recovery records must be objects")
        require(
            recovery.get("viewport") == "1440x900",
            f"{recovery.get('id')}: recovery viewport must be 1440x900",
        )
        require(
            recovery.get("disposition") == "N/A_SOURCE_TEST",
            f"{recovery.get('id')}: recovery must remain source-tested/N/A",
        )

    validate_acceptance(manifest)
    print(
        f"PASS {path}: M1E5 accepted Main visual/runtime evidence valid; physical Windows 4K "
        "readability deferred to #223"
    )
