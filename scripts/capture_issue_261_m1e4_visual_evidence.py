...
def require_sha(value: Any, label: str, *, allow_pending: bool = True) -> None:
    ...
    require(
        ...
        f"{label} must be a SHA-256 hex digest or null pending value",
    )
...
def check_planned_artifact(entry: dict[str, Any], label: str) -> None:
    require(
        ("path" in entry or "path_pattern" in entry) and "sha256" in entry and "status" in entry,
        f"{label} needs path/path_pattern, sha256 and status",
    )
    if "path_pattern" in entry:
        ...
        if viewport_hashes is not None:
            require(
                isinstance(viewport_hashes, dict)
                and set(viewport_hashes) == set(REQUIRED_VIEWPORTS),
                f"{label}.sha256_by_viewport must cover all five viewports",
            )
    ...
    if entry["status"] == "PENDING_MAIN_CAPTURE":
        ...
        require(
            isinstance(entry.get("path") or entry.get("path_pattern"), str),
            f"{label} needs a path once captured",
        )
...
def check_region_map(pair: dict[str, Any], family: str) -> None:
    ...
    require(
        names == REQUIRED_CROPS, f"{family} region_map must cover exactly {sorted(REQUIRED_CROPS)}"
    )
    for item in regions:
        ...
        require(
            isinstance(matches, list), f"{family} region target_selector_matches must be a list"
        )
        if item["region"] == "native-preview" and family != "MOD-EXPORT":
            ...
            require(
                isinstance(proof, dict) and proof.get("match_count") == 0,
                f"{family} native-preview needs machine N/A proof",
            )
...
def check_source_pairs(payload: dict[str, Any]) -> None:
    ...
    require(
        {item.get("id") for item in pairs if isinstance(item, dict)} == set(FAMILIES),
        "static source pairs must cover MOD-DATA/PROCESS/FIT/EXPORT exactly",
    )
    for pair in pairs:
        family = pair["id"]
        ...
        require(
            isinstance(reference, dict) and isinstance(current, dict),
            f"{family} needs reference and current source records",
        )
        ...
        require_text(
            reference.get("identity_classification"), f"{family} reference identity classification"
        )
        ...
        require_text(
            current.get("identity_classification"), f"{family} current identity classification"
        )
        originals = reference.get("originals")
        require(
            isinstance(originals, list) and originals, f"{family} reference originals are required"
        )
        ...
        for side in ("reference", "current"):
            ...
            require(
                {entry.get("region") for entry in crops} == REQUIRED_CROPS,
                f"{family}.{side}.crops must cover all required regions",
            )
            for index, entry in enumerate(crops):
                if side == "reference" and entry.get("status") == "APPROVED_STATIC_REFERENCE":
                    require_sha(
                        entry.get("sha256"),
                        f"{family}.reference.crops[{index}].sha256",
                        allow_pending=False,
                    )
                else:
        ...
        require(
            isinstance(current_originals, list) and len(current_originals) == 2,
            f"{family} current before/after originals are required",
        )
        require(
            {entry.get("phase") for entry in current_originals} == {"before", "after"},
            f"{family} current originals must have before and after",
        )
        ...
        require(
            isinstance(differences, list) and len(differences) >= 3,
            f"{family} needs explicit pre-existing difference dispositions",
        )
...
def check_state_matrix(payload: dict[str, Any]) -> None:
    ...
    require(
        set(by_id) == EXPECTED_STATE_IDS,
        "capture state matrix is missing or has unapproved state IDs",
    )
    for state_id, item in by_id.items():
        ...
        require(
            isinstance(viewports, list) and viewports,
            f"{state_id}.required_viewports must be non-empty",
        )
        ...
        if state_id == "materials-browse-lineage-readback":
            ...
            require(
                matches == [] and isinstance(proof, dict) and proof.get("match_count") == 0,
                f"{state_id} needs zero-match selector proof",
            )
            ...
            require(
                viewports == ["1440x900"],
                f"{state_id} evidence-only state must be captured at 1440x900",
            )
            require(matches, f"{state_id} must prove target selector matches")
...
def check_runtime_contract(payload: dict[str, Any]) -> None:
    ...
    require(
        contract.get("schema_version") == "cmp.issue-261.m1e4.runtime-identity.v1",
        "runtime identity schema drifted",
    )
    ...
    required_fields = {
        ...
        "test_data_revision",
        "process_output_id",
        "process_output_revision_id",
        "process_output_sha256",
        "fit_output_id",
        "fit_output_revision_id",
        "fit_output_sha256",
        "material_model_id",
        "material_model_revision_id",
        "material_model_ir_sha256",
        "solver_card_id",
        "solver_card_revision_id",
        "mapping_digest",
        "native_preview_sha256",
        "download_sha256",
    }
    require(
        isinstance(fields, dict) and set(fields) == required_fields,
        (
            "runtime identity server_returned_fields must cover every "
            "ID/revision/digest/resource field"
        ),
    )
    require(
        all(value is None for value in fields.values()),
        "writer may not freeze runtime UUIDs or digests",
    )
    resources = contract.get("resources")
    require(
        isinstance(resources, list) and len(resources) >= 10,
        "runtime identity must list every read-back API resource",
    )
    for resource in resources:
        ...
        require(
            resource.get("read_back_assertion") is not None,
            "runtime resources need exact read-back assertions",
        )
    assertions = contract.get("exact_within_run_assertions")
    require(
        isinstance(assertions, list) and len(assertions) >= 12,
        "runtime identity needs exact within-run/read-back assertions",
    )
    ...
    require(
        ...
        and {"latest", "first", "global-output", "other-session"}.issubset(set(forbidden)),
        "runtime contract must prohibit all fallback classes",
    )
...
def check_quality_contract(payload: dict[str, Any]) -> None:
    ...
    require(
        isinstance(interaction.get("tab_order"), list) and len(interaction["tab_order"]) >= 6,
        "keyboard tab order is incomplete",
    )
    require(
        isinstance(interaction.get("action_reachability"), list)
        and len(interaction["action_reachability"]) >= 8,
        "keyboard action reachability is incomplete",
    )
    require(
        isinstance(interaction.get("focus_preservation"), list)
        and len(interaction["focus_preservation"]) >= 4,
        "focus preservation records are incomplete",
    )
    require(
        interaction.get("page_horizontal_overflow") == 0, "page overflow contract must require zero"
    )
    ...
    for axis in (
        ...
        "engineering_task_flow",
        "responsive_wide_screen_composition",
    ):
    ...
    require(
        isinstance(empty_series, dict),
        "EngineeringCurvePlot EmptyCompatibleSeries contract is required",
    )
    require(
        empty_series.get("story") == "EngineeringCurvePlot EmptyCompatibleSeries",
        "Storybook story identity drifted",
    )
    require(
        empty_series.get("viewport") == "1440x900"
        and empty_series.get("device_pixel_ratio") == 1
        and empty_series.get("browser_zoom_percent") == 100,
        "Storybook EmptyCompatibleSeries must be exactly 1440x900 at DPR1/100%",
    )
    expected_storybook = "PASS_MAIN_PIXEL_EXACT" if accepted else "PENDING_MAIN_CAPTURE"
    require(
        empty_series.get("evidence") == expected_storybook, "Storybook story disposition drifted"
    )
    if accepted:
...
def check_q_matrix(payload: dict[str, Any]) -> None:
    ...
    require(
        set(by_id) == {f"Q{index:02d}" for index in range(1, 21)},
        "Q01..Q20 records are required exactly once",
    )
    for item in records:
        ...
        allowed = (
            {"PASS_MAIN", "EXPLICIT_NA"}
            if payload.get("status") == "ACCEPTED_MAIN_VISUAL_AND_RUNTIME"
            else {"PENDING_MAIN", "EXPLICIT_NA"}
        )
        require(
            item.get("disposition") in allowed,
            f"{item['id']} disposition is invalid for the manifest lifecycle",
        )
        if item["disposition"] == "EXPLICIT_NA":
            require(
                item.get("selector_scan", {}).get("match_count") == 0,
                f"{item['id']} N/A needs zero selector matches",
            )
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    require(
        payload.get("contract_schema") == "cmp.issue-261.m1e4.visual-evidence.v5",
        "manifest must use approved packet v5 schema",
    )
    require(payload.get("viewports") == REQUIRED_VIEWPORTS, "manifest viewport matrix drifted")
    require(
        payload.get("browser_zoom_percent") == 100 and payload.get("device_pixel_ratio") == 1,
        "manifest must pin browser zoom 100% and DPR 1",
    )
    require(
        payload.get("capture_plan", {}).get("required_crops") == sorted(REQUIRED_CROPS),
        "manifest crop contract drifted",
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="validate the contract without touching evidence files",
    )
    ...
    source_only = sum(
        item.get("evidence") == "ACCEPTED_DURABLE_SOURCE_ORACLE_ONLY" for item in matrix
    )
    pending = sum(item.get("evidence") == "PENDING_MAIN_CAPTURE" for item in matrix)
    print(
        f"{payload['unit']}: manifest PASS; schema={payload['contract_schema']}; "
        f"states={len(matrix)}; accepted={accepted}; source-only={source_only}; "
        f"pending={pending}"
    )
    ...
    if not args.check_only:
        print(
            "No capture was started. Main must run the approved disposable "
            "Compose/browser workflow."
        )
    return 0
