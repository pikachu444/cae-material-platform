...
- Visual implementer packets name the exact approved assets/screens/components, user task, preserved
  ...
  judgment before product-owner approval. A rejected local CSS approach gets one
  correction/re-review by default and must not be repeated. If that re-review still fails, stop and
  ask the product owner with the exact findings; only explicit authorization opens one second bounded
  correction/re-review, and a third correction is forbidden.
