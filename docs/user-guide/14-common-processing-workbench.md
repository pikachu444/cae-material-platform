...
백엔드는 여러 DMA 등온선을 하나의 처리 결과로 저장하고, 계산용(CALIBRATION)과 검증용(HOLDOUT)
자료를 구분해 Prony 입력으로 제공합니다. API에서 초기 설정을 추천받을 수 있지만 소재별로 검증된
기본값은 아닙니다. 추천 정보가 없던 기존 저장 결과도 다시 계산하거나 변경하지 않고 사용할 수 있습니다.
#392의 UI 후보는 철회했으며, 새 처리 화면은 frontend 개편에서 구현합니다. 따라서 이 API 지원이
현재 화면에서 다중 주파수 처리 업무를 완료할 수 있다는 뜻은 아닙니다. 한 주파수 sweep 안에서는 대표 온도와
