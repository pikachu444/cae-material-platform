...
`make demo-e2e`)는 실제 브라우저에서 Materials 검색, Modeling과 Bulk Export Center가 열리는지
확인합니다. 첫 실행에는 Chromium 설치가 포함될 수 있습니다.
...
6. Abaqus와 OpenRadioss mapping 상태를 확인한 뒤 native preview에서 `.inp`와 `.rad`를 내려받습니다.
