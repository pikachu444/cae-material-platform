...
- **Library**: 이미 등록된 exact Test Data revision을 선택합니다.
...
3. CSV, TSV 또는 XLSX 파일을 선택하고 **Inspect source**를 실행합니다. 확장자와 실제 파일
   설정은 이후 Profile에 함께 고정됩니다.
...
4. **Preview on graph**를 실행해 등록 전 curve와 original/normalized unit을 확인합니다.
5. 그래프가 의도한 시험을 나타낼 때 **Register reviewed data**를 실행합니다.
...
**Register reviewed data**는 승인한 exact Profile로 Import Run을 먼저 실행합니다. 성공하면
