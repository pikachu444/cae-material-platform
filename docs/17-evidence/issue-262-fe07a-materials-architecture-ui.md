...
## Bounded consistency correction

The visible `Source & history` page now uses the concrete heading `Related records and workflow` and omits the generic instruction paragraph. `Related Records`, `Workflow` and `Open exact datasheet` are retained. The Curves disclosure summary now reads `Curve source`. The internal `evidence` route/state key, exact URL/session pins, related-record behavior and Materials-to-Modeling contracts are unchanged; there is no broader copy sweep or visual redesign.

The [consistency manifest](images/issue-262-fe07a-materials-architecture-ui/owner-correction/consistency-correction/manifest.json) records the two affected states at 1366×768, 1440×900, 1920×1080, 2560×1440 and 3840×2160 with zoom 100% and DPR 1. It contains 10 before/after original pairs, 70 direct 100%-pixel crops and 90 PNGs total. Every after measurement has zero page horizontal overflow. Main opened all 10 after originals and all 35 after crops at original resolution; the new labels remain visible, the removed paragraph is absent, the retained record/workflow tables and exact-datasheet action remain reachable, and no overlap, clipping or wide-screen regression is present. The five corrected Curves originals are also promoted to the registered current guide captures.

The consistency-stage owner packet contained exactly five distinct 1920×1080 originals in the required order at that stage: Search, Browse, Detail Overview, Curves and active Source & history. Only Curves and Source & history changed in that correction; the other three states were recaptured to preserve that report contract. Carbon information hierarchy passed through concrete source/workflow labels, the COMSOL-style engineering task flow retained exact related-record and curve-source access, and SAP-style responsive composition remained unchanged across the five CSS viewports. Physical Windows 4K remains #223.

Affected verification is pass: 2 focused frontend files and 9 tests, all five guided-demo scenarios, production TypeScript/Vite build with zero bundle warnings, targeted Ruff, five-viewport geometry assertions, current-guide promotion and manifest integrity. The canonical independent Balanced follow-up reviewer returned `APPROVED` with no material findings.

## Owner-approved Browse, lineage and Curves correction

Browse now presents `Name | Code | Category | Description | Revision`. The Name cell contains only the record name; Code contains the exact Catalog external key or an em dash. The Code column remains unsortable because the existing server/query contract does not advertise code sorting. Row click/Enter selection, exact revision navigation, Search/Browse URL state and existing APIs are unchanged. Original-resolution review found and corrected an initial long-code ellipsis; the final browser gate verifies that every visible Code cell fits without clipping at all five viewports.

The default `Source & history` surface is now `Linked records`, backed only by the existing direct incident Record Links. Its visible fields are Relation, Target record, Type and Exact revision. The existing computed multi-hop graph is unchanged and available behind one collapsed `Full lineage` disclosure; no workflow entity, API, schema or backend behavior was added or changed. Internal route/state key `evidence`, exact pins and linked-datasheet actions remain intact.

Curves no longer repeats `Declared curve contract`, the left-list `View only` state or the sentence that statistical/envelope curves are view-only. One `View only` state remains in the active curve header. Channel quantities, axes, units, `No deviation recorded` and the collapsed `Curve source` disclosure remain visible and unchanged.

The [bounded correction manifest](images/issue-262-fe07a-materials-architecture-ui/owner-correction/browse-links-curves-correction/manifest.json) records Browse, Curves and Source & history at 1366×768, 1440×900, 1920×1080, 2560×1440 and 3840×2160. It contains 15 before/after pairs and 100 direct 100%-pixel crops at zoom 100%/DPR 1. All 15 after measurements have zero page horizontal overflow. Main opened all 15 after originals and all 50 after crops at original resolution. Carbon-level hierarchy passes through the explicit Name/Code and direct-link field grammar; COMSOL-style task flow keeps direct exact navigation primary and moves multi-hop inspection into a disclosure; SAP-style composition keeps the Navigator bounded while results, linked rows and the graph use available width without clipping or route-specific scaling. Physical Windows 4K remains #223.

The current manifest owner packet contains exactly five 1920×1080 originals: Browse, Test Data detail, Simulation Data detail, Solver Card detail and Curves. Internal Source & history and the other four viewports remain evidence-only. The deterministic flow still verifies Search/Browse selection, exact Material and Record revision pins, direct Test Data and Processing Output hops, browser Back/Forward/Reload, exact Material/State Modeling handoff, and Solver Card target/version/unit/release/preview/download state. The bounded fixture gaps recorded above remain unchanged; no missing multi-hop link or Modeling source was inferred.

The final type-specific correction removes stored demo descriptions and qualified record names from the normal Test Data, Simulation Data and Solver Card headers without changing Catalog identity. Simulation Data retains `Type Processing Output` and shows only its populated result type, input condition, result and exact source fields. Solver Card places target, format, unit system, release state, exact revision, structured review state, acknowledgement and download controls before the exact native card text. Its preview is taller at FHD, remains locally scrollable and exposes one accessible expand/collapse control. Artifact comments remain unchanged API content.

Main recaptured and reopened the exact 1920×1080 Simulation Data and Solver Card originals after this correction. `Simulation Data` is the visible h1 and no test/specimen/curve-coverage empty rows remain. The Solver Card original contains no instructional review sentence, shows `Review required | Post-necking extension` with the short `Reviewed` checkbox, keeps metadata and download visible, and gives the native preview the dominant above-the-fold area. The existing bounded viewport gate then passed 15 geometry records across 1366×768, 1440×900, 1920×1080, 2560×1440 and 3840×2160 with the owner packet regenerated from the same live Compose state.

Final affected verification is pass: 4 focused frontend files and 17 tests, production TypeScript/Vite build and bundle budget, the focused guided exact-link browser test, the canonical exact-record continuity run including Back/Forward/Reload, Modeling handoff and Solver Card review/preview/download state, guide integrity, documentation impact and diff hygiene. The same canonical Balanced reviewer found two coupled consistency defects in its first pass; Main corrected solver-heading precedence and the stale default capture assertions, reran the affected gates, and the reviewer returned `APPROVED` on the corrected result. No backend, API or schema contract changed.
