...
The manifest's `ownerReport` is the final owner-facing packet and contains exactly five distinct 1920x1080 originals, in this order: Search, Browse, Detail Overview, Curves and active Source & history. The Source & history capture verifies the visible tab is selected while the preserved internal route key remains `evidence`. Other viewports, direct crops and error/recovery states remain internal evidence and are not part of the owner report packet. The canonical Balanced reviewer reopened these five originals and the updated manifest and returned `APPROVED` with no material findings.
...
The final owner packet was regenerated and still contains exactly five distinct 1920×1080 originals in the required order: Search, Browse, Detail Overview, Curves and active Source & history. Only Curves and Source & history changed in this bounded correction; the other three states were recaptured in the packet to preserve the exact report contract. Carbon information hierarchy passes through concrete source/workflow labels, the COMSOL-style engineering task flow retains exact related-record and curve-source access, and SAP-style responsive composition remains unchanged across the five CSS viewports. Physical Windows 4K remains #223.
