...
| Actions | In Modeling → Process, open the optional Distribution analysis sheet, inspect the saved Selection/Plan/Run, compare the empirical CDF with all fitted candidates, select Normal, enter an explicit reason, save, reload, and reopen the sheet. |
| Visible outcome | The ordinary Process workspace remains unchanged until requested. The analysis sheet gives its remaining space to the probability graph, with a shallow command bar and one candidate ledger for Normal, Lognormal and Weibull support, parameters, AICc, delta AICc, BIC, AD, estimator-aware bootstrap p, refit counts, state and warnings. Recommendation and selected model remain visibly separate. |
...
- The mounted surface is an optional full-workspace analysis sheet inside the current
  `MaterialModelingWorkspace`; it is not a route, completion screen, bottom dock or third inspector.
  The underlying Process workspace remains mounted and inert while the sheet is open, then returns
  unchanged on Close or Escape. The sheet resolves pinned revisions through immutable Dataset
  history, keeps historical processed Selections visible after a head advances, excludes normalized
  source Selections, reuses an exactly matching immutable Plan, and keeps replay/provenance in
  bounded disclosures.
...
| Web scalar/chart focus and responsive tests | PASS — 24 focused tests, including plotted-path validity, modal focus cycling and saved-selection reload |
...
five exact-main before originals, ten live after originals across Standard and Large, and thirty
direct 100%-pixel crops at original resolution. Chromium 151 used browser zoom 100% and DPR 1. Page
horizontal overflow and candidate-table local overflow are zero at every viewport and density. The
last Weibull row, saved decision and utility disclosures remain reachable at 1366×768, including
Large. The covered Process plot stays mounted at 322px or taller, is inert/hidden from assistive
technology while the modal sheet is open, and returns without state mutation.

Q01–Q03 pass: the unchanged Process navigator, exact revision identities and compact engineering
rows remain available when the optional sheet closes. Q04–Q11 pass: a ResizeObserver-driven
empirical/fitted CDF graph owns elastic analysis space, the ledger stays aligned below it, direct row
selection opens one rationale editor, focus cycles inside the modal sheet and Escape returns to the
trigger. Q12–Q19 are not applicable because Export and Administration are unchanged. Q20 passes
deterministic geometry at 1920/2560/3840 for Standard and Large with no page overflow, CSS zoom,
transform scaling, fabricated filler or route-specific 4K override.
