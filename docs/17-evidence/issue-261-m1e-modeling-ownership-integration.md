...
- The regenerated inventory records 10 remaining legacy rows with an owned same-selector peer: seven pre-existing boundaries plus CSS-0148, CSS-0612, and CSS-0614, the three M4 shared-shell rows that the explicit compact correction must cascade with. Those M4 rows remain in legacy ownership and are not counted as M1E2 moves.
...
The capture fixture cannot truthfully synthesize four server-dependent or deliberately hidden states. They are recorded as test-only contracts rather than screenshots:

- stale Recipe conflict: exact 412 append with `If-Match`, visible alert, and Reload/Keep/Cancel actions;
- stale Material/State family context: no substitution of a current head for a stale URL revision;
- hidden top-level Saved outputs support drawer: DOM and disclosure characterization while the core layout intentionally hides that companion;
- uncalculated Process plot: draft/history/current preservation and deferred reconciliation until Material context resolves.

The JSON names the exact component tests for each contract. Neither phase contains an invalid state record or browser/console error.
...
The comparison authority is `docs/17-evidence/images/issue-261-m1e2-modeling-core-shell/before-after-comparison.json`. The generated CSS oracle also passed for `apps/web/dist` and `apps/web/storybook-static`: all 101 owner rows / 95 source groups are emitted with the expected selector/context multiplicities and no missing or unexpected duplicate moved selector.
...
- Affected core-shell component tests: 46/46 passed.
- Full web regression: 71 files, 413/413 tests passed; its frontend-guard unit suite also passed 17/17.
...
The first independent read-only review found one blocker: production reserved only one 34 px flex row while the 900 px navigator rendered a second row, clipping Export. The explicit compact correction and recaptured evidence resolve that finding. Independent re-review passed with no actionable findings; the reviewer reopened both routes at 860, 861, and 900 px plus their navigator/graph crops at original resolution and returned `approve / PASS` for Q-01 through Q-20 with the recorded N/A items.
...
This evidence authorizes no merge. Publication is limited to the owner-authorized local commit, branch push, and Draft PR; Issue #261 remains open for the 383 residual M1E rows and later migration batches.
