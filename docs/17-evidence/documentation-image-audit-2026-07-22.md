...
- Preserve PR-specific duplicate image bytes when they are part of an immutable historical evidence
  set; current recaptures receive independent bytes and stable current paths.
...
- replaced the generation-mixed 36-entry current manifest with those 20 stable current captures and
  registered 103 retained historical captures in the separate archive;
...
- kept ten current-to-history duplicate hash groups deliberately. They are independently owned
...
The checker now treats a duplicate inside one lifecycle as an error, reports permitted
current-to-history equality, verifies image magic against filename suffixes, and fails on orphaned
images. This preserves historical proof without allowing silent duplicate accumulation.

## 8. Final verification

| Check | Result |
| --- | --- |
| Live capture | 20/20 completed from `http://127.0.0.1:5173`; exact viewport, no pending async state and no horizontal overflow |
...
| Current manifest | 20 unique current captures; dimensions and `CURRENT_CAPTURE_OUTPUTS` ownership verified |
| Historical archive | 103 unique entries; dimensions and source-evidence paths verified |
...
| Image inventory | 225 images; 0 suffix/byte mismatches; 0 orphans; 10 permitted cross-lifecycle duplicate groups |
| Contract regression | `8 passed` for user-guide and documentation-impact contracts |
...
| Change hygiene | Ruff and `git diff --check` passed; documentation impact reports 0 visual source changes |
