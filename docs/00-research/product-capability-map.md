...
| Saved Recipe library 및 batch | versioned recipe를 저장·수정·재사용하고 선택한 Dataset에 preflight 후 batch 실행한다. 성공 Output을 모델로 승격하면 exact Recipe/Batch/Member/Attempt pin을 유지한다. | `implemented` | exact-profile Recipe identity/revisions/ordered steps + immutable exact-input Batch/Member/Attempt + migration 080 promotion origin | Recipe lifecycle, member preflight/execute/retry, exact Output origin resolution | Recipe Library/Batch Monitor + polymer IR execution evidence/deep link | domain/API/migrations 065~066/080/React/live PostgreSQL/browser | `FR-BAT-001~007`, `T-54/T-69`; durable queued scale-out은 최적화 범위다. |
