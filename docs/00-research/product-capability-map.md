...
| Material Workflow Explorer | Material → State → Test/Specimen → Dataset → Processing → IR → Card → Release를 exact revision link로 투영한다. | `mischaracterized` | migration 061은 configurable Record끼리의 exact graph만 저장하며 실제 Material/State/Test/IR/Card revision binding은 없다. | bounded Record graph API | Record graph/deep link | configurable Record forward/reverse regression | `FR-NAV-002`, `T-51/T-62` |
...
| 금속 탄소성 modeling | 다섯 E 선택, proof stress, 비파괴 necking 후보, true/plastic 변환, Voce/Swift/Hockett–Sherby/Ghosh fitting, 두 후보 조합과 bounded extrapolation을 Recipe method로 구현했다. selected Output은 exact Test/Profile lineage와 함께 IR `1.2.0`으로 승격되며 같은 Material workbench에서 Abaqus/OpenRadioss card로 이어진다. | `partial` | explicit Output/Test/Profile/selection/domain columns, composite FK, guard trigger, curve Artifact | metal methods, commit/batch, exact Output promotion, IR/curve, preflight/card | 후보/선택 overlay, exact Output 선택, IR lineage, 두 solver preview/download | analytical/deterministic/domain/API/migration/React/live Docker/PostgreSQL/card download | 계산·기존 IR/card는 구현됨. `FR-MOD-M-001/006/007`이 요구하는 method/조합/외삽의 canonical Neutral JSON 보존은 `T-63` |
| 폴리머 점탄성 modeling | 공통 Recipe에서 log-time 처리와 1~10항 generalized-Maxwell 후보 비교, BIC/수동 선택을 수행한다. exact Selection master curve는 manual/WLF/Arrhenius를 지원한다. | `partial` | Processing Output과 기존 별도 linear-Prony IR 저장소 | processing candidate와 기존 수동/reviewed IR API가 분리됨 | processing과 기존 IR/card 화면이 병존 | 수치/API/UI 회귀 | selected common Output을 IR/Neutral JSON으로 승격하는 계약이 없고 demo card는 수동 baseline에서 생성된다. OpenRadioss linear-Prony는 명시적 `unsupported`. `FR-MOD-P-001~003`, `T-55P/T-63` |
| 엘라스토머 초탄성/초점탄성 modeling | exact multi-test revision을 네 공개식에 fitting하고 reviewed family를 canonical Neutral JSON과 두 solver card로 연결한다. | `partial` | hyperelastic Candidate/diagnostics와 Neutral typed projection | family run/promotion/preflight/card | connected comparison/promotion/export | equation/API/PostgreSQL/React | hyperelastic family Neutral JSON은 구현됐지만 `FR-MOD-E-004`의 optional Prony overlay는 기존 별도 Ogden–Prony IR에만 있고 canonical family 승격에서 보존되지 않는다. `T-55E/T-63` |
| Neutral Material JSON | `cmp.neutral-material`은 metal, linear-viscoelastic, hyperelastic/hyper-viscoelastic selected model을 같은 versioned envelope로 교환해야 한다. | `partial` | migration 071은 hyperelastic Candidate/Plan/FK와 coefficient 열에 고정 | hyperelastic promote/import/get/download만 지원 | hyperelastic family 선택 화면만 연결 | hyperelastic schema/domain/API/migration/browser | 현재 1.0.0 parser와 import 검증이 hyperelastic 전용이다. 금속·폴리머 및 Prony overlay union은 `T-63`에서 구현한다. `FR-JSON-007~009`, `T-56/T-63` |
