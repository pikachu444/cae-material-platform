...
| 사용자 매뉴얼과 GUI evidence | 깨끗한 Compose seed가 금속·폴리머·엘라스토머 reference 자료를 만들고 Dashboard의 세 안내 경로, protected API 검증 명령, unified walkthrough와 screenshot gate를 제공한다. | `implemented` | migration 074 head에서 clean seed 검증 | protected seed + `make demo-verify` | guided Dashboard와 전체 user/admin guide | clean Compose, API verifier, React, guide/screenshot gate | `FR-UX-001~003`, `T-60` |
