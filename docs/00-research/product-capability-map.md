...
| 폴리머 점탄성 modeling | 공통 Recipe의 log-time/1~10항 후보 비교와 기존 bounded two-term reviewed Prony calibration을 제공한다. exact Output Artifact, selected terms, BIC/RMSE와 catalog G₀ 불일치를 사람이 검토한 뒤 generalized-Maxwell IR과 canonical Neutral JSON으로 승격한다. | `implemented` | migration 079 typed Processing Output evidence, exact Test/Profile/Property pins, 1~10 ordered IR/Neutral Prony rows | Output→reviewed IR→Neutral→Abaqus Prony 또는 조건부 OpenRadioss LPRONY card; Artifact server revalidation | exact Output 선택, 사례별 G₀ mismatch limit, review, evidence/curve, shared mapping/card UI와 approximation 확인 | 수치/domain/API/schema/migration/React/clean PostgreSQL/live Docker verifier | Abaqus와 공통 1~10항 Output 승격은 ADR-0031/T-67, nearly-incompressible shear-only OpenRadioss `/VISC/LPRONY`는 ADR-0032/T-68이다. LAW62 silent 변환과 production qualification은 제외한다. `FR-MOD-P-001~005`, `T-55P/T-63/T-64/T-67/T-68` |
