...
| 폴리머 점탄성 modeling | 공통 Recipe의 log-time/1~10항 후보 비교와 기존 bounded two-term reviewed Prony calibration을 제공한다. exact Output Artifact, selected terms, BIC/RMSE와 catalog G₀ 불일치를 사람이 검토한 뒤 generalized-Maxwell IR과 canonical Neutral JSON으로 승격한다. | `implemented` | migration 079 typed Processing Output evidence, exact Test/Profile/Property pins, 1~10 ordered IR/Neutral Prony rows | Output→reviewed IR→Neutral→Abaqus Prony card; Artifact server revalidation | exact Output 선택, 사례별 G₀ mismatch limit, review, evidence/curve, shared mapping/card UI | 수치/domain/API/schema/migration/React/clean PostgreSQL/live Docker verifier | Abaqus 재생성과 공통 1~10항 Output 승격은 ADR-0031/T-67로 완료했다. OpenRadioss linear-Prony는 명시적 `unsupported`이고 production qualification은 제외한다. `FR-MOD-P-001~004`, `T-55P/T-63/T-64/T-67` |
