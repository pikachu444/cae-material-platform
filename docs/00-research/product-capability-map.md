...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Configurable Table 및 Attribute Definition | Granta MI 공개 object model과 Attribute 관리 기능을 참고한다. 관리자가 migration 없이 scalar, curve, file, link attribute를 정의한다. | `partial` | identity/revision + 9 typed value tables | protected create/list/revise | technical schema designer; product Administration pending | unit/API/PostgreSQL/UI | `FR-CFG-001~005`, `T-49/T-78` |
| Layout, Subset, datasheet | Granta의 Layout/Subset 개념을 참고하되 독자 UI로 구현한다. Layout 순서로 typed Record를 작성하고 검색 조건을 Subset revision으로 저장·재적용한다. | `partial` | revision + ordered items/filter + typed values | create/list/revise/apply | isolated datasheet; persistent record workspace pending | API/PostgreSQL/UI | `FR-CFG-006~007`, `T-49/T-50/T-77` |
...
| Catalog Explorer | Database/Profile → Table → nested Folder → Record를 persistent Contents Tree로 제공해야 한다. 현재 한 Table 아래 레코드를 평면 나열하고 workflow를 카드 목록으로 표시하므로 Granta 유사 완료 주장은 맞지 않는다. | `mischaracterized` | reusable Folder/Record/Subset engine | low-level lazy children/search API | flat `/catalog/explorer`; replacement pending | current tests prove engine only | `FR-NAV-001/003/006`, `T-51/T-71/T-76` |
| Material Workflow Explorer | Material → State → Test/Specimen → Dataset → Processing → IR → Card → Release를 exact revision link로 계층 투영해야 한다. | `mischaracterized` | exact binding/link engine implemented | bounded graph API implemented | flat node/card list; hierarchical projection pending | tests prove binding, not tree usability | `FR-NAV-002`, `T-51/T-62/T-65/T-66/T-76` |
| Typed arbitrary record link | 관리자가 Link Type, 방향명, 허용 Table, cardinality를 정의하고 양 끝 revision을 고정한다. | `partial` | explicit Link Type/Link identity+revision tables | protected create/revise/query API | technical editor; datasheet context integration pending | endpoint/cardinality/latest/cross-scope/immutability | `FR-LNK-001~005`, `T-51/T-77/T-78` |
| Search, facet, compare, saved subset | configurable Record의 이름·설명·text Attribute 검색, Folder, discrete facet, normalized number range, exact revision compare와 saved Subset을 제공한다. | `partial` | typed search indexes | bounded search/compare | isolated technical screen; AMDC-style results/compare pending | API/PostgreSQL/React | `FR-NAV-003~006`, `T-50/T-77` |
| Canonical Test Data JSON | JSON을 공식 교환 형식으로 하고 CSV/TSV/XLSX는 같은 구조로 변환하는 adapter가 된다. 원본 JSON과 digest를 보존한다. | `partial` | typed identity/revision/condition/channel + JSON/Parquet Artifact pins | validate/adapter/import/revise/list/exact/package export | technical import page; Material Database/Modeling integration pending | schema/domain/API/React/live PostgreSQL round-trip | `FR-JSON-001~006`, `T-52/T-79` |
| Mapping Profile | 자유 Attribute/채널을 계산 quantity에 연결하는 revisioned profile이다. | `partial` | explicit identity/revision + typed channel/Attribute binding, exact Attribute Definition pin | create/list/get/revise + strong ETag | JSON editor exists; visual Map step pending | domain/API/fresh PostgreSQL/React/live browser | `FR-MAP-001~004`, `T-53/T-79` |
| Common Processing Workbench | crop, scale/shift, resample, smoothing, curve alignment/statistics를 ordered method pipeline으로 구성하고 단계별 overlay와 scalar result를 봐야 한다. 현재 기능이 technical page에 분산되어 cohesive Material Modeler형 workbench 완료 주장은 맞지 않는다. | `mischaracterized` | exact input/profile/Output engine | preview/commit methods | disconnected technical screens | numeric/API tests prove engine only | `FR-PRO-001~010`, `T-53/T-79` |
...
| 단순 사용자 권한 | Administrator/User와 기능 grant engine은 있으나 일반 화면에 token/API가 노출되고 관리 화면이 제품 구성과 분리되어 있다. | `partial` | Migration 074 typed assignment | `/product-access/*` | technical `/access`; product Administration pending | unit/API/PostgreSQL/UI | `FR-ACC-001~005`, `T-59/T-75/T-78` |
| 사용자 매뉴얼과 GUI evidence | 기존 seed/download 증거는 engine을 검증하지만 visible token setup, flat demo와 direct deep-link 중심이라 제품 사용성 완료를 증명하지 않는다. | `mischaracterized` | engine seed retained | verifier retained | replacement demo/manual pending | new home-to-card E2E required | `FR-UX-001~009`, `T-60/T-65/T-82/T-83` |
