...
| Common Processing Workbench | crop, scale/shift, resample, smoothing, curve alignment/statistics를 ordered method pipeline으로 구성하고 단계별 overlay와 scalar result를 본다. | `partial` | exact input/profile/Output engine | preview/commit methods | server graph exists; direct manipulation/axes/tools/session shell are pending | numeric/API/Vitest/live Docker/browser | `FR-PRO-001~010`, `T-53/T-79/T-85/T-86` |
