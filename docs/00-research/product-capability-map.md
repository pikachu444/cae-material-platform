...
| Common Processing Workbench | crop, scale/shift, resample, smoothing, curve alignment/statistics를 ordered method pipeline으로 구성하고 단계별 overlay를 본다. | `implemented` | exact input/profile FK + one-revision Output/step + Artifact pin | seven-method preview, ensemble alignment/statistics, commit/list/download | exact Test Data/profile/steps + shared-axis stage/member/statistics overlay + Output list/download | numeric/domain/API/migration/React/live Docker/browser | `FR-PRO-001~010`, `T-53`; reusable Recipe/batch promotion is T-54 |
| Saved Recipe library 및 batch | versioned recipe를 저장·수정·재사용하고 선택한 Dataset에 preflight 후 batch 실행한다. 현재 alignment batch는 일반 Recipe batch가 아니다. | `partial` | exact-profile common Recipe identity/revisions/ordered steps + bounded legacy batch | Recipe create/list/get/revise/publish; general batch missing | connected Recipe Library; Batch Monitor missing | domain/API/migration/React/live Docker | `FR-BAT-001~006`, `T-54`; exact Selection/preflight/run-attempt/retry remain |
