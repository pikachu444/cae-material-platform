...
| 금속 탄소성 modeling | T-55M 1차로 OLS/Huber/chord/secant/manual E, offset proof stress, 비파괴 necking 후보, 명시적 engineering→true/plastic 변환을 Recipe 호환 method로 추가했다. tabulated plasticity/reference Voce와 두 solver card는 기존에 있다. 복수 hardening fitting·후보 조합·bounded extrapolation은 남아 있다. | `partial` | 처리 Output/Recipe가 scalar와 변환 curve를 고정 | 4개 metal method registry/preview/commit/batch | Processing Workbench scalar 결과와 curve stage | public-equation numeric/domain/API/React; live evidence 추가 중 | `FR-MOD-M-001~004` 1차 구현, `FR-MOD-M-005~007` 및 `T-55M` 후속 |
