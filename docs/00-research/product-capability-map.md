...
| Canonical Test Data JSON | JSON을 공식 교환 형식으로 하고 CSV/TSV/XLSX는 같은 구조로 변환하는 adapter가 된다. 원본 JSON과 digest를 보존한다. | `partial` | typed identity/revision/condition/channel + JSON/Parquet Artifact pins | validate/import/list/exact export | connected `/datasets/test-json` | schema/domain/API/React/live PostgreSQL round-trip | `FR-JSON-001~006`, `T-52` |
