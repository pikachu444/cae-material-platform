...
| 금속 탄소성 modeling | T-55M 1·2차로 다섯 E 선택, proof stress, 비파괴 necking 후보, true/plastic 변환과 Voce/Swift/Hockett–Sherby/Ghosh 동일-objective fitting, 두 후보 조합, bounded extrapolation을 Recipe method로 구현했다. lower/initial/fitted/upper, RMSE와 observed/extrapolated domain이 Output에 고정된다. tabulated/reference Voce와 두 solver card는 기존에 있으나 새 selected Processing Output의 IR 승격 연결은 남아 있다. | `partial` | exact Output/Recipe/Batch가 scalar, 후보·선택 curve와 domain을 고정 | 5개 metal method registry/preview/commit/batch | 후보 4개+선택 조합 overlay, parameter/metric scalar | analytical/deterministic/domain/API/React/live Docker/PostgreSQL | `FR-MOD-M-001~007` 처리 구현, `T-55M` IR/card 연결 후 완료 |
