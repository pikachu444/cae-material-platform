...
| Common Processing Workbench | crop, scale/shift, resample, smoothing, curve alignment/statistics를 ordered method pipeline으로 구성하고 단계별 overlay를 본다. | `partial` | immutable output missing | seven-method registry + composed preview | exact Test Data/profile/steps + shared-axis stage overlay | numeric/domain/API/React/live browser | `FR-PRO-001~010`, `T-53`; output/alignment/statistics remain |
