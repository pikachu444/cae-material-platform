...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Configurable Table 및 Attribute Definition | Granta MI 공개 object model과 Attribute 관리 기능을 참고한다. 관리자가 migration 없이 scalar, curve, file, link attribute를 정의한다. | `implemented` | identity/revision + 9 typed value tables | protected create/list/revise | T-92 guided Database design administration accepted | unit/API/PostgreSQL/Vitest/live browser | `FR-CFG-001~005`, `T-49/T-78/T-92` |
| Layout, Subset, datasheet | Granta의 Layout/Subset 개념을 참고하되 독자 UI로 구현한다. Layout 순서로 typed Record를 작성하고 검색 조건을 Subset revision으로 저장·재적용한다. | `implemented` | revision + ordered items/filter + typed values | create/list/revise/apply | T-91 three-pane Datasheet, domain tabs and exact revision context accepted | API/PostgreSQL/Vitest/live browser | `FR-CFG-006~007`, `T-49/T-50/T-77/T-91` |
...
| Catalog Explorer | Database/Profile → Table → nested Folder → Record를 persistent Contents Tree로 제공한다. | `implemented` | Folder/Record/Subset와 stable domain binding | lazy children/search API | T-91 realistic hierarchy, automatic useful entry, keyboard disclosure and session restoration accepted | nested lazy expansion/live PostgreSQL/browser | `FR-NAV-001/003/006`, `T-51/T-71/T-76/T-91` |
| Material Workflow Explorer | Material → State → Test/Specimen → Dataset → Processing → IR → Neutral → Card를 exact revision link로 계층 투영한다. | `implemented` | exact binding/link revision engine | depth-eight bidirectional graph API | T-91 Catalog/Workflow projection and exact workbench navigation accepted | link pin advance/reverse/deep-link/live browser | `FR-NAV-002`, `T-51/T-62/T-65/T-66/T-76/T-91` |
| Typed arbitrary record link | 관리자가 Link Type, 방향명, 허용 Table, cardinality를 정의하고 양 끝 revision을 고정한다. | `implemented` | explicit Link Type/Link identity+revision tables | protected create/revise/query API | T-91 exact Related/Revisions navigation and T-92 guided Link Type administration accepted | endpoint/cardinality/latest/cross-scope/immutability/live browser | `FR-LNK-001~005`, `T-51/T-77/T-78/T-91/T-92` |
| Search, facet, compare, saved subset | configurable Record의 이름·설명·text Attribute 검색, Folder, discrete facet, normalized number range, exact revision compare와 saved Subset을 제공한다. | `implemented` | typed search indexes | bounded search/compare | T-92 typed search/facet/range, saved Subset and Layout comparison audit accepted | API/PostgreSQL/Vitest/live browser | `FR-NAV-003~006`, `T-50/T-77/T-92` |
...
| Saved Recipe library 및 batch | versioned recipe를 저장·수정·재사용하고 선택한 Dataset에 preflight 후 batch 실행한다. | `implemented` | Recipe/Batch engine implemented | lifecycle/preflight/execute/retry | T-92 published restore, clone/revise/publish, family-filtered member preflight and attempt monitor accepted | domain/API/PostgreSQL/Vitest/live browser | `FR-BAT-001~007`, `T-54/T-69/T-70/T-80/T-92` |
...
| 폴리머 점탄성 modeling | Relaxation log-time/Prony/WLF/Arrhenius와 DMA storage/loss joint Prony fitting을 graph-centered family track에 연결했다. | `implemented` | exact JSON/Profile/Recipe/Batch | relaxation/DMA fitting, IR/Neutral promotion and dual-solver export | response/residual/candidate/term UI and same-surface Card accepted in T-89 | numeric/backend/PostgreSQL/Vitest/live browser | `FR-MOD-P-001~005`, `T-55P/T-67~T-69/T-80/T-81/T-89` |
| 엘라스토머 초탄성/초점탄성 modeling | exact multi-mode/holdout Dataset, saved Plan, four-family fitting, eight-candidate diagnostics, stability/uncertainty와 reviewed Neutral/Card를 같은 track에서 실행한다. | `implemented` | exact Plan/Run/Candidate/Prony evidence | Plan/fitting/promotion/export connected | T-90 accepted automatic exact restore, family rail, response/residual graph, Prony overlay and two card previews | numeric/backend/Vitest/live Docker browser | `FR-MOD-E-001~004`, `T-55E/T-72/T-80/T-81/T-90` |
| Neutral Material JSON | `cmp.neutral-material`은 metal, linear-viscoelastic, hyperelastic/hyper-viscoelastic selected model을 같은 versioned envelope로 교환한다. | `implemented` | typed three-family engine implemented | promotion/validate/import/download implemented | Metal T-88, Polymer T-89 and Elastomer T-90 same-session review/download accepted | schema/domain/API/migration/numeric/Vitest/live browser | `FR-JSON-007~009`, `T-56/T-63/T-64/T-81/T-88~T-90` |
| Solver mapping과 native card | 공식 solver 문서 기반 manifest, six-state preflight, ASCII/sidecar를 제공한다. | `implemented` | typed export engine implemented | exact-Neutral preflight/report/card implemented | Metal T-88, Polymer T-89 and Elastomer T-90 preview/download paths accepted; solver execution remains excluded | golden/domain/API/migration/UI/live browser | `T-57/T-64/T-81/T-88~T-90` |
| JSON+ZIP bulk package | exact Test JSON, Mapping Profile, Recipe, Neutral JSON, mapping report와 native card를 기존 immutable Selection/Job/Bundle 엔진으로 조립한다. revision-addressed path, manifest, checksum과 omission을 보존한다. | `implemented` | package engine implemented | discover/select/job/bundle/download | T-93 clean verifier reproduced 19/24-component checksum bundles from exact sources | domain/API/migration/checksum/UI/clean verifier | `FR-EXP-005~006`, `T-58/T-81/T-88/T-93` |
| 단순 사용자 권한 | User/Reviewer/Administrator task preset을 제품 어휘로 제공하고 세밀한 resource/action/scope enforcement는 내부 확장점으로 유지한다. | `implemented` | Migration 074 typed assignment + 090 Reviewer preset | `/product-access/*` | integrated Users & access without token/API/policy vocabulary | unit/API/PostgreSQL/Vitest/live browser | `FR-ACC-001~006`, `T-59/T-75/T-78` |
| 사용자 매뉴얼과 GUI evidence | 자동 demo session, 계층 Material Database, Layout Datasheet/search/compare와 세 family Modeling 및 reviewed delivery journey를 실제 화면으로 문서화했다. | `implemented` (bounded reference product) | clean hierarchy/schema seed reproduced | clean verifier passed | T-93 Dashboard/Database/three-family Modeling/Card browser acceptance captured | full frontend/Python/PostgreSQL/clean browser | `FR-UX-001~009`, `T-60/T-65/T-75~T-93` |
