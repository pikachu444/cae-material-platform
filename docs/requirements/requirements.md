...
This requirements catalog preserves raw/input/output bytes, units and quantity semantics, typed
input/output relations, authorization, scientific validation and release meaning. Its current target
has one domain revision boundary: Material information edits create immutable Material information
revisions containing identity, associated state/manufacturing/heat-treatment and direct property values.
State/PropertySet identities and Specimen, TestRun, TestData, Dataset, Selection, Mapping Profile,
Process, Model, Solver Card and Link records are stable-ID saved objects; renames preserve links and do
not stale science. A saved result retains its actual typed inputs and settings as result data.

Universal Entity–Activity–Agent provenance, per-edit save reasons, snapshot proliferation and hidden
nonmaterial revision writers are not requirements. Concrete result, artifact, review, validation and
release contracts may retain the evidence and responsible action they need. Software/schema/file
versions, hashes and opaque concurrency tokens are metadata. The nonmaterial `revision` wording in
legacy rows below is superseded by this boundary until the real DB/API migration; it must not be used to
add a new history writer. D0 is documentation only; the existing OpenAPI and runtime remain legacy
compatibility behavior until a later RD migration unit.
...
- `MAY`: 후속 확장이다.
- 모든 write API는 security/request metadata로 사용자/서비스 agent, organization, request/trace ID를
  기록한다. 이 기록은 domain revision이나 universal Entity–Activity–Agent history가 아니다.
- 모든 계산 결과는 실제로 사용한 typed input, option/setting과 필요한 Material information
  revision을 결과 데이터로 보존한다. 입력 변경은 current eligibility를 갱신하지만 저장된 결과 bytes를
  다시 쓰거나 nonmaterial revision을 만들지 않는다.
...
| ID | 요구사항 | 검수 기준 |
| --- | --- | --- |
| `FR-CFG-001` | 관리자는 migration 없이 Table과 typed Attribute Definition saved object를 추가해야 한다. | 새 Attribute가 즉시 record form, validation과 search schema에 나타난다. |
...
| `FR-CFG-004` | required/default/validation/parameter/conditional visibility를 Attribute Definition saved object에 고정해야 한다. | 저장된 record 값은 새 definition으로 재해석되거나 덮어쓰이지 않고, 현재 입력 eligibility가 명시된다. |
| `FR-CFG-005` | 대형 curve/file은 immutable artifact를 참조하고 row-per-point 또는 opaque record JSON을 authority로 사용하지 않아야 한다. | digest와 schema를 통해 saved record/Test Data에서 원본 artifact까지 추적한다. |
...
| `FR-NAV-002` | Workflow Explorer는 Material에서 Release까지 typed stable-object links를 투영한다. Catalog node 역조회는 Catalog publication이 적용되는 경우에만 사용하며, ordinary Test Data와 Solver Card는 권한이 있으면 직접 연다. | Explorer 노드에서 exact workbench로 이동하고 Material/Test JSON/Processing/Neutral 화면에서 같은 graph로 돌아가 관련 saved object를 열 수 있으며 `latest`, sibling name 또는 Catalog publication을 관계 전제로 저장하지 않는다. |
| `FR-NAV-003` | 전체 text와 typed Attribute facet/range 검색을 지원해야 한다. | unit-normalized 수치 범위와 권한 필터가 count/facet에도 동일하게 적용된다. |
| `FR-NAV-004` | 여러 record를 선택한 Layout으로 비교해야 한다. | 값, 단위, 출처와 Material information revision 차이를 한 화면에 표시한다. |
...
| `FR-LNK-001` | 관리자는 방향명, 허용 source/target Table과 cardinality를 가진 stable Link Type definition을 정의해야 한다. | 정의에 맞지 않는 endpoint와 개수는 거부된다. |
| `FR-LNK-002` | record link 양 끝은 stable saved-object IDs를 고정해야 한다. Material information revision이 실제 계약에 필요한 경우에만 concrete revision을 함께 저장한다. | 이름 변경이나 현재 eligibility 변경에도 명시적 link가 가리키는 대상과 저장된 결과가 변하지 않는다. |
| `FR-LNK-003` | cross-organization/project 또는 classification 역전 link를 거부해야 한다. | service와 PostgreSQL negative test가 모두 통과한다. |
| `FR-LNK-004` | link supersede/remove는 관계의 stable saved-object 상태와 concrete result/release evidence를 보존해야 한다. | 적용되는 lifecycle 계약에서만 과거 관계 상태를 재구성하고, 일반 link edit가 domain revision을 만들지 않는다. |
...
| ID | 요구사항 | 검수 기준 |
| --- | --- | --- |
| `FR-CAT-001` | Material의 안정 identity와 Material information revision을 분리해야 한다. | 이름·조성 등 Material information 변경이 기존 revision을 덮어쓰지 않고 새 revision을 만든다. |
| `FR-CAT-002` | Material State를 Material과 분리하고 State identity는 stable saved object로 관리해야 한다. | 열처리·수분·노화·물리적 상태를 시험조건과 혼동하지 않고 Material information revision에 연결해 참조한다. |
| `FR-CAT-003` | Manufacturing Process Definition과 실제 Process Run을 분리한 stable saved objects로 관리해야 한다. | 공정 recipe와 실제 실행 날짜·장비·조건·operator가 별도 entity로 조회된다. |
| `FR-CAT-004` | Lot과 Batch를 구분하고 batch input/output 관계를 stable typed links로 표현해야 한다. | 하나의 batch가 여러 lot을 소비하거나 lot의 일부가 여러 batch로 갈라지는 예를 저장한다. |
| `FR-CAT-005` | Specimen을 물리적 stable identity로 관리해야 한다. | geometry, orientation, source lot/batch, preparation, conditioning history를 추적한다. |
| `FR-CAT-006` | Test Method, Test Campaign, Test Run, Test Condition을 분리한 stable saved objects로 관리해야 한다. | 같은 method를 여러 campaign/run이 사용하며 각 TestRun의 condition snapshot이 고정된다. |
| `FR-CAT-007` | Instrument와 instrument calibration reference를 TestRun에 연결해야 한다. | run 시점의 장비·교정 상태를 조회할 수 있다. |
| `FR-CAT-008` | 도메인별 metadata는 plugin schema로 확장해야 한다. | core migration 없이 새 시험 metadata schema를 등록·검증하고 schema/version은 metadata로 보존한다. |
...
| `FR-ING-005` | Importer는 detect와 import를 분리해야 한다. | detect 결과를 사용자가 확인한 뒤 선택한 stable Mapping Profile object와 mapping snapshot으로 import한다. |
...
| `FR-ING-010` | 첫 governed tabular importer는 CSV, TSV, XLSX의 sheet/header/encoding/locale을 명시해야 한다. | detect/preview 이후 승인된 Mapping Profile 또는 explicit mapping snapshot만 import하며 spreadsheet formula는 data로 실행하지 않는다. |
...
| `FR-UNIT-004` | Unit Profile은 Catalog Profile·Mapping Profile과 별도의 stable saved object여야 한다. | 입력·표시·solver export 선택을 exact profile ID와 저장된 profile data로 다시 읽는다. |
| `FR-UNIT-005` | Processing, Fit와 Export의 profile 사용 결과는 실제 적용 위치와 사용 설정을 함께 고정해야 한다. | 결과에서 location, role, quantity semantics, dimension과 unit ID를 actual input/settings와 함께 조회한다. |
...
| `FR-JSON-004` | 원본 JSON bytes와 digest를 보존하고 내부 Parquet 변환을 별도 result/artifact로 기록해야 한다. | JSON export와 계산 artifact가 각각 실제 input/output 관계와 필요한 evidence로 나타난다. |
...
| `FR-JSON-010` | 모든 exchange document는 organization/project/classification과 stable object references를 포함해야 한다. 적용되는 Material information revision과 concrete result input은 명시한다. | cross-scope import와 unresolved reference가 거부된다. |
| `FR-MAP-001` | Mapping Profile은 record Attribute와 test channel을 calculation quantity에 연결하는 stable saved object여야 한다. | profile 변경이 과거 run에 저장된 actual mapping input을 바꾸지 않는다. |
...
| `FR-MAP-003` | 사용자는 mapping을 확인·수정하고 별도 Profile saved object 또는 explicit mapping snapshot으로 저장해야 한다. | UI와 API가 같은 validation report를 표시한다. |
| `FR-MAP-004` | 계산 Run은 사용한 Mapping Profile ID와 actual mapping input/settings를 결과에 보존해야 한다. | profile 변경 후에도 이미 저장된 run의 입력과 결과 재현 정보가 같다. |

### 2.3 Dataset, saved results and evidence

| ID | 요구사항 | 검수 기준 |
| --- | --- | --- |
| `FR-DAT-001` | Dataset은 stable identity를 가진 saved object이며 새 처리 결과는 별도 Dataset/output object로 저장해야 한다. | 새 처리 결과가 기존 Dataset이나 saved result bytes를 수정하지 않는다. |
...
| `FR-DAT-004` | 계산 입력 집합은 stable Selection saved object와 저장된 membership/input snapshot으로 고정해야 한다. | specimen 추가/제외 후에도 과거 calibration이 사용한 입력을 정확히 재현한다. |
| `FR-DAT-005` | concrete result/artifact는 계약에 필요한 actual typed input, output, settings와 evidence를 가져야 한다. | 해당 contract completeness 검사에서 고아 산출물과 누락된 필수 evidence가 거부된다. Universal activity graph는 요구하지 않는다. |
| `FR-DAT-006` | Material information revision과 concrete result/input-output relation을 구분해야 한다. | Material history와 processing output/link가 별도 typed relation으로 조회되고 일반 편집이 `wasRevisionOf`를 만들지 않는다. |
| `FR-DAT-007` | typed links를 upstream/downstream 양방향으로 조회해야 한다. | card에서 raw asset까지, raw asset에서 연결된 release까지 탐색하되 graph traversal을 derivation으로 추정하지 않는다. |
...
| `FR-STA-005` | curve alignment/resampling을 명시적 Processor operation으로 실행해야 한다. | Statistical Analyzer가 숨은 interpolation으로 원본 곡선을 바꾸지 않고, 결과가 사용한 operation/settings를 보존한다. |
...
| `FR-STA-007` | lot/batch/orientation/rate/temperature 등 strata 비교를 지원해야 한다. | grouping 차원을 바꾼 analysis가 별도 stable saved result로 저장된다. |

### 2.5 전처리와 보정

| ID | 요구사항 | 검수 기준 |
| --- | --- | --- |
| `FR-PRO-001` | processing recipe를 ordered step과 parameter를 가진 stable saved object로 저장해야 한다. | smoothing, cropping, offset, conversion 등 모든 step이 manifest에 있다. |
| `FR-PRO-002` | preview와 committed run을 구분해야 한다. | preview artifact는 release의 concrete result input으로 사용할 수 없다. |
...
| `FR-PRO-006` | common workbench는 crop, scale/shift, resample, moving-average, Savitzky–Golay, spline, alignment와 통계를 ordered step으로 제공해야 한다. | 단계별 overlay와 committed output object가 일치한다. |
| `FR-PRO-007` | 사용자는 기존 Recipe를 불러와 수정하고 별도 saved Recipe object로 저장해야 한다. | 기존 saved Recipe와 published artifact는 수정되지 않고 새 draft object가 추가된다. |
...
| `FR-BAT-001` | Batch는 ordered Dataset Selection, Mapping Profile과 Recipe saved-object IDs 및 사용 snapshot을 고정해야 한다. | 실행 중 현재 정의가 바뀌어도 member 입력은 변하지 않는다. |
| `FR-BAT-002` | 실행 전 member별 compatibility preflight를 제공해야 한다. | incompatible member와 이유가 계산 전에 표시된다. |
| `FR-BAT-003` | member 결과는 독립 Processing Run/Output saved object여야 한다. | 한 member 실패가 성공 member를 롤백하거나 덮어쓰지 않는다. |
| `FR-BAT-004` | 실패한 member만 동일 입력으로 재실행할 수 있어야 한다. | retry가 성공 결과를 중복 생성하지 않고 attempt를 기록한다. |
| `FR-BAT-005` | batch 결과는 success/failure/warning과 output object를 항목별로 제공해야 한다. | API와 Batch Monitor count가 일치한다. |
| `FR-BAT-006` | deterministic Recipe는 동일 입력에서 tolerance 내 동일 결과를 내야 한다. | batch regression fixture가 재현된다. |
| `FR-BAT-007` | Recipe Batch가 만든 Output을 모델로 승격할 때 exact Recipe/Batch/Member/Attempt/Output saved-object IDs와 actual settings를 끝까지 보존해야 한다. | IR evidence와 Neutral JSON의 Recipe reference가 성공한 Attempt의 Output과 일치하지 않으면 DB와 API가 거부한다. |
...
| `FR-CAL-008` | 같은 Material Model을 반복 보정할 때 prior promotion evidence를 보존해야 한다. | 각 saved result/model object가 실제 input/settings와 evidence를 보유하고 과거 IR/Card/Release digest가 변하지 않는다. |
...
| `FR-MOD-M-006` | fitting 선택 또는 두 후보의 명시적 구간/비율 조합을 별도 curve result object로 저장해야 한다. | 조합 방식과 범위가 Neutral JSON에 재현된다. |
...
| ID | 요구사항 | 검수 기준 |
| --- | --- | --- |
| `FR-IR-001` | 모든 production card는 released 또는 release-candidate Material Model IR saved object에서 생성해야 한다. | raw parameter dict에서 exporter를 직접 호출하는 API가 없고, card result가 실제 IR input을 보존한다. |
...
| `FR-EXP-005` | 사용자는 선택한 test data, neutral IR, mapping report와 card를 immutable Bulk Export Bundle로 받을 수 있어야 한다. | archive의 manifest/checksum이 모든 파일과 exact source object/input을 검증하고 누락·미지원 항목을 조용히 제외하지 않는다. |
...
| ID | 요구사항 | 검수 기준 |
| --- | --- | --- |
| `FR-VAL-001` | Validation Template은 geometry, mesh, BC, loading, output extraction, metric을 가진 일반 saved definition으로 관리해야 한다. | validation contract가 요구하는 concrete Template object를 result input으로 고정하고, Validation Run이 해당 결과에 사용한 실제 geometry, mesh, BC, loading, output extraction, metric inputs를 보존하며, 일반 Template 편집은 독립 domain revision을 만들지 않는다. |
...
| `FR-VAL-004` | 수동 실행 결과 반입과 managed runner 실행을 동일 Result Manifest로 표현해야 한다. | 두 경로의 concrete input/output, artifact and validation-evidence completeness가 동일하다. Universal provenance graph는 요구하지 않는다. |
...
| `FR-WF-003` | release 전 concrete input/evidence, unresolved issue and mapping policy를 자동 검사해야 한다. | 하나라도 실패하면 release가 생성되지 않는다. Universal provenance graph는 요구하지 않는다. |
...
| `FR-ACC-006` | 제품 역할은 User, Reviewer, Administrator로 제공해야 한다. User와 Administrator는 검색·조회·다운로드와 upload/review request, processing/fitting/card request를 수행하고, Reviewer는 이를 포함해 material/card 변경 요청·승인·publish를 수행한다. Administrator는 review request를 만들고 자신의 Activity 항목을 복구하지만 다른 사용자의 승인 결정을 내리지 않는다. | 역할별 task preset, Reviewer-only decision authorization, PostgreSQL versioned preset constraint 및 RLS 회귀가 함께 통과한다. |
...
| `FR-UX-006` | Record는 Layout 기반 Datasheet를 중심으로 Properties, Curves, Test Data, Models, CAE Cards와 Links를 한 문맥에서 제공해야 한다. | 검색과 tree에서 연 record가 동일 datasheet를 사용하고 Material information revision 또는 concrete result input은 보조 정보로 표시된다. |
...
| `NFR-REP-002` | release의 concrete input, artifact, validation/review evidence completeness check는 누락 0건이어야 한다. Universal domain provenance completeness는 요구하지 않는다. |
...
| `NFR-PERF-003` | 10 hop 또는 10,000 edge 이내 typed-link/evidence traversal p95 < 2 s를 목표로 한다. |
...
| `NFR-DR-003` | backup restore drill을 정기 실행하고 restore된 artifact digest와 concrete result/input evidence를 검증한다. |
