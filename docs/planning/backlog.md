...
- 순서 12 `#189 lazy chunk 예산과 Workbench 분할 기준`은
  ...
  `b9a82e96532fe796aaf1889387cd391c952e1c67`에서 완료했습니다. Task 2는
  [#341](https://github.com/pikachu444/cae-material-platform/issues/341), [PR #345](https://github.com/pikachu444/cae-material-platform/pull/345),
  main `c52eaf29b63b45669eeef7b33af8cf91279b4fbc`에서 완료했고, Task 1B는
  [#342](https://github.com/pikachu444/cae-material-platform/issues/342), [PR #353](https://github.com/pikachu444/cae-material-platform/pull/353)에서
  구현·검증·독립 감수와 2026-08-28 제품 소유자 화면 승인을 완료했습니다. #343 Task 2B도
  [PR #356](https://github.com/pikachu444/cae-material-platform/pull/356), main
...
| 26 | [x] [#246 source-v2 원본 정합과 누락 범위 폐쇄](https://github.com/pikachu444/cae-material-platform/issues/246) — Task 1A [PR #250](https://github.com/pikachu444/cae-material-platform/pull/250), main `b9a82e96532fe796aaf1889387cd391c952e1c67`; Task 2 [#341](https://github.com/pikachu444/cae-material-platform/issues/341), [PR #345](https://github.com/pikachu444/cae-material-platform/pull/345), main `c52eaf29b63b45669eeef7b33af8cf91279b4fbc`; Task 1B [#342](https://github.com/pikachu444/cae-material-platform/issues/342), [PR #353](https://github.com/pikachu444/cae-material-platform/pull/353), main `fa4e451f5000f4610b817e490e5b7e133892a5a9`; Task 2B [#343](https://github.com/pikachu444/cae-material-platform/issues/343), [PR #356](https://github.com/pikachu444/cae-material-platform/pull/356), main `712bda0b5c8680844d53bd27cd7295dbb0e52208`; Task 3+4 [#344](https://github.com/pikachu444/cae-material-platform/issues/344), [PR #357](https://github.com/pikachu444/cae-material-platform/pull/357), main `1dcd4c90ec8636bc66de46961cbb93a8392fda47` | 원본 포맷을 실제로 수용하고 완료 Issue 밖에 남은 요구를 중복 없이 폐쇄 | issue, 원본 패키지, 요구사항 추적표, #204~#216 현재 코드·계약 | Task 1A source adapter, Task 2 common-unit, Task 1B exact JSON 등록, Task 2B DMA 지원 경계, Task 3+4 후속 소유권·보류 결정을 모두 완료해 #246을 닫았다. 현재 첫 미완료 제품 단위는 #195다. |
| 27 | [#195 production 폴리머 점탄성 Fit](https://github.com/pikachu444/cae-material-platform/issues/195) | 기존 synthetic/reference 계산 기반 위에서 승인된 DMA/relaxation 입력, 수치 acceptance, 후보 비교·선택·저장·reload와 실제 사용자 흐름을 production 계약으로 완성 | issue와 current planning packet, #158/#209/#246, 당시 최신 API·계산·UI, 제품 scope gate | #195는 열린 multi-unit issue로 유지한다. #390 분해에 따라 #391 → #392 → #380 순서로 진행하며, 각 child unit의 구현·시각 acceptance·publication이 남아 있다. |
| 27-1 | [x] [#391 multi-frequency DMA TTS backend](https://github.com/pikachu444/cae-material-platform/issues/391) — [PR #396](https://github.com/pikachu444/cae-material-platform/pull/396), main `6abddd25b9ef8e7f2e0a8d112feaf3846996379b` | 여러 backend 입력을 exact multi-isotherm 결과로 계산·그룹화·저장하고 Prony 입력을 제공 | #390/#391 issue, processing contract, multi-frequency kernel, persistence/read-back tests | multi backend calculation/grouping/persistence/exact GET/Prony resolver 완료. 다음 child unit은 #392 |
| 27-2 | [ ] [#392 DMA TTS Process UI](https://github.com/pikachu444/cae-material-platform/issues/392) — active implementation | exact governed multi-frequency source를 분류·추천·검토·저장하고 exact read-back 이후 Fit에 연결 | #390/#392 issue, #391 merged contract, Process/Fit UI, frontend architecture와 visual evidence gates | 현재 구현 단위. strict classifier, read-only recommendation, one-save/exact GET gate, multi Process rail/results, fixed compatibility와 회귀·문서가 범위다. 완료 뒤 #380 |
| 27-3 | [#380 DMA TTS Export/solver-card live program](https://github.com/pikachu444/cae-material-platform/issues/380) — next | #392에서 검증된 saved TTS/Prony 결과를 Export·solver-card 흐름에 연결 | #390/#380 issue, #392 saved-result contract, Export/solver-card acceptance | #392 완료 후 시작하며 현재 범위에서는 구현하지 않는다. |
| 28 | [#211 representative envelope와 approved Fit input](https://github.com/pikachu444/cae-material-platform/issues/211) | 기존 mean/95% CI와 append-only 포함·제외 lineage를 재사용해 p05/p95 대표 revision을 검토·승인하고 승인된 exact revision을 Fit에 사용 | issue, #160/#184, #206/#210/#246, 현재 alignment·Statistics/QC·calibration scope·Fit 계약 | 기존 common-grid piecewise-linear/no-extrapolation, exact Dataset/Test Run lineage, outlier 판단, immutable mean/95% CI와 exact input pinning 회귀검증; 새 p05/p95 representative revision·review/approval/invalidation·approved representative exact revision→Fit selection 검증, merge |
| 29 | [#213 governed solver-card Template 기반](https://github.com/pikachu444/cae-material-platform/issues/213) | 기존 Export 결과를 보존하며 검토된 Template로 안전하게 render | issue의 sandbox ADR gate, #160/#184/#205/#246, #158 Export | renderer 호환성, isolation, exact Template/Mapping provenance, merge |
| 30 | [#214 LS-DYNA MAT_024·다중 단위·Template UI](https://github.com/pikachu444/cae-material-platform/issues/214) | released Template로 MAT_024와 여러 Unit Profile 카드를 생성·관리 | issue, #160/#184/#205/#213, solver mapping/Administration | golden·unit·review·preview/download checksum·다섯 viewport·기존 solver 회귀, merge |
| 31 | [#215 SPA OIDC Code+PKCE](https://github.com/pikachu444/cae-material-platform/issues/215) | production SPA login·logout·expiry/권한 오류를 안전하게 복구 | issue, #160/#184/#246, identity/security 계약과 deployment config | provider·negative·role·production bypass E2E·다섯 viewport, merge |
| 32 | [#216 제품 command audit wiring](https://github.com/pikachu444/cae-material-platform/issues/216) | 주요 command의 성공·거절·복구를 기존 hash chain에서 누락 없이 추적 | issue, #160/#184/#246, 필요 시 #213/#215, Audit 계약 | event matrix, atomicity, redaction, integrity·restore 검증, merge |
| 33 | [#162 Ubuntu VM·문서 최종 검증](https://github.com/pikachu444/cae-material-platform/issues/162) | 사용자 설치 방식과 분리된 Hyper-V Ubuntu 최종 통합검증 harness에서 전체 흐름과 역할별 복구가 재현되고 현재 문서와 화면이 일치 | issue, 병합된 기능·화면, #381에서 정리된 current 문서 목록과 문서 계약; Windows 11 native/offline 제품 경로는 독립 #321 | Hyper-V Ubuntu VM의 깨끗한 최종 통합검증, 전체 흐름·역추적, current 문서/화면 일치와 문서 계약 검증, merge. 이를 사용자 제품 설치·배포 방식으로 선언하지 않음 |
| 34 | [#162 공개 실측 데이터 최종 검증](https://github.com/pikachu444/cae-material-platform/issues/162) | 인터넷에서 확보한 실제 측정 샘플을 제품에 등록해 솔버 카드와 Materials 재조회까지 완료 | issue의 고정 NIST Numisheet 2020 파일·공식 해시·이용조건, 병합된 실제 제품 흐름 | 원본 바이트·출처 보존, 실제 등록·처리·여러 모델·선택·카드 생성·검토·승인·재조회·다운로드·원본 역추적 자동 확인, 최종 승인, merge |
| 35 | [#223 제품 전체 실제 Windows 4K 최종 물리 가독성 검증](https://github.com/pikachu444/cae-material-platform/issues/223) | Materials·Modeling·Activity·Administration을 실제 Windows 4K 100%·150%·200%에서 읽고 조작할 수 있음을 최종 확인하고 #184의 fixture 증거 부채를 닫음 | issue, #221 잠정 정책, #184 전역 적용·handoff matrix·누락 30개 목록, #160 Activity 증거, #162 작업 2 완료 상태 | #184에서 인계한 정확한 30개 원본을 세 density로 재수집하고 structured manifest와 독립 원본 재감수를 완료. 이어 세 Windows 배율의 환경 메타데이터·대표 route/state 원본·1:1 crop, 물리 가독성·상호작용·복구·read-back 검증, 제품 소유자 최종 판정, 발견 결함 bounded bug와 재검증, merge |
