...
| 27 | [#195 production 폴리머 점탄성 Fit](https://github.com/pikachu444/cae-material-platform/issues/195) | 기존 synthetic/reference 계산 기반 위에서 승인된 DMA/relaxation 입력, 수치 acceptance, 후보 비교·선택·저장·reload와 실제 사용자 흐름을 production 계약으로 완성 | issue와 current planning packet, #158/#209/#246, 당시 최신 API·계산·UI, 제품 scope gate | bounded backend 단위는 explicit relaxation/governed shear-DMA 입력, 고정 주파수 온도 sweep→명시적 TTS→Prony, 독립 numerical oracle, Plan/Run/Candidate/Recommendation/Selection, PostgreSQL save/reload·stale 검증까지 현재 작업 브랜치에서 완료. `non_production`은 유지하며 별도 UI·시각 acceptance와 publication이 남음 |
| 27-1 | [x] [#391 다중 주파수 DMA TTS 백엔드](https://github.com/pikachu444/cae-material-platform/issues/391) — [PR #396](https://github.com/pikachu444/cae-material-platform/pull/396), main `6abddd25b9ef8e7f2e0a8d112feaf3846996379b` | TTS 계산·저장·재조회와 Prony 입력 제공 | #391 API·계산·저장 계약 | main 반영 완료 |
| 27-2 | [#392 DMA TTS 후속 작업](https://github.com/pikachu444/cae-material-platform/issues/392) — [PR #397](https://github.com/pikachu444/cae-material-platform/pull/397) 백엔드 보존 | 추천 API, 저장 결과 검증, Prony 입력 연결 개선을 보존 | 2026-09-07 제품 소유자 지시, #391 계약 | 실패한 UI 후보는 철회한다. PR #397은 백엔드와 필요한 계약·테스트만 병합하며 frontend 완료를 뜻하지 않는다. 기존 저장 결과 호환성을 검증한다. 다음 우선 작업은 별도 frontend 개편의 조회·다운로드 대표 업무이며 DMA 처리 화면은 그 후 재설계한다. #392와 상위 #195는 미완료 상태로 유지한다. |
